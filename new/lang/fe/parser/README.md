...
- `asm.mach` — inline assembly. Parses the optional ISA tag, the operand list (`name: in`, `name: out`, `name: clobber`, with optional `= source` rename), and collects the raw body text by tracking brace depth through the already-lexed token stream.
