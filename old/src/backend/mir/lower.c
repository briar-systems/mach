...
static bool mir_lower_x86_64(const Target *target, MirModule *module, CodegenResult *result);

bool backend_mir_lower(const Target *target, MirModule *module, CodegenResult *result)
{
    if (!target || !module || !result || !target->isa)
    {
        return false;
    }

    switch (target->isa->kind)
    {
    case TARGET_ISA_KIND_X86_64:
        return mir_lower_x86_64(target, module, result);
    default:
        return false;
    }
}
...
static bool add_text_label(X86Emitter *ctx, const char *name)
{
    ...
    return backend_label_list_add(&ctx->result->labels, name, SECTION_TEXT, offset);
}

static bool add_rodata_label(X86Emitter *ctx, const char *name)
{
    size_t offset = codebuf_offset(&ctx->result->rodata.buffer);
    return backend_label_list_add(&ctx->result->labels, name, SECTION_RODATA, offset);
}
...
static bool emit_rel32_placeholder(X86Emitter *ctx, const char *label)
{
    ...
    return backend_reloc_list_add(&ctx->result->relocs, SECTION_TEXT, offset, RELOC_X86_64_PC32, label, -4);
}
        ...
        if (data->kind == MIR_DATA_VAL && data->init_value.string_value)
        {
            ...
            for (size_t i = 0; src[i] != '\0'; i++)
            {
                if (src[i] == '\\' && src[i + 1] != '\0')
                {
                    ...
                    {
                    case 'n':
                        codebuf_emit_byte(&ctx->result->rodata.buffer, '\n');
                        break;
                    case 't':
                        codebuf_emit_byte(&ctx->result->rodata.buffer, '\t');
                        break;
                    case 'r':
                        codebuf_emit_byte(&ctx->result->rodata.buffer, '\r');
                        break;
                    case '\\':
                        codebuf_emit_byte(&ctx->result->rodata.buffer, '\\');
                        break;
...
static bool emit_instruction(X86Emitter *ctx, MirInstruction *inst)
{
    ...
    case MIR_OP_ARCH_SYSCALL:
        // emit isa-specific syscall instruction
        if (ctx->target->isa && ctx->target->isa->emit_syscall)
        {
            ctx->target->isa->emit_syscall(buf);
        }
        break;
    case MIR_OP_RET:
