...
| boot/src/commands/cmd_build.c | src/commands/build.mach | ⚠️ | `--target` ignored; default output path; module root/dep resolution incomplete; loaded module cap now bumped to 256. |
| boot/src/commands/cmd_test.c | src/commands/testing.mach | ❌ | Stub: no AST transform / test harness generation; just runs existing binary. |
| boot/src/commands/cmd_run.c | src/commands/run.mach | ⚠️ | Target lookup TODO; default binary path only. |
| boot/src/commands/cmd_init.c | src/commands/init.mach | ⚠️ | Does not add `mach-std` dep like bootstrap; templates differ. |
| boot/src/commands/cmd_dep.c | src/commands/dep.mach | ⚠️ | `dep tidy` not implemented; no version pinning. |
...
| boot/src/compiler/sema.c | src/compiler/sema.mach | ⚠️ | Lvalue checks TODO; control-flow pass TODO; string-building placeholders. |
...
| boot/src/compiler/masm/isa/x86_64/isel.c | src/compiler/masm/isel.mach | ⚠️ | Needs parity for vreg sizing + mem base/index handling + odd-size aggregate moves. |
...
- ⏭ Next: implement full test harness in `src/commands/testing.mach` by porting `boot/src/commands/cmd_test.c`.
- ⏭ Next: implement target-aware build/run output paths (`mach.toml` targets) in `build.mach` / `run.mach`.
- ⏭ Next: vreg multi-slot allocation in `src/compiler/masm/isel.mach`.
