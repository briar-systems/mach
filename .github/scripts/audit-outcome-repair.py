...
baseline = "b3d216f659e82d2b4aaeedd6d0cb34158ea62df2"
...
def test(name, selected, count=1, child=0):
    ...
    row = dict(name=name, filter=selected, counts=counts, exits=exits, compiler_exit=rc, verified=valid, failures=re.findall(r"^\s*FAIL\s+(\S+) .*\(exit", log, re.M))
    results.append(row)
...
try:
    ...
    restore()
    path = "src/lang/fe/parser/state.mach"
    for name in ["sync_to", "sync_to_body", "sync_to_decl", "error_at_span"]:
        current = (root / path).read_text(encoding="utf-8")
        before = function(current, name)
        after = before.replace("    if (p.fatal) { ret; }\n", "")
        assert before != after
        mutate_one(path, before, after)
    test("old-recovery-after-fatal", "mach.lang.editor.parse:driver_and_editor_share_one_parse_outcome", child=5)
    ...
    mutate_one(path, before, after)
    test("old-raw-session-source-key", "mach.lang.session.load_source:source_and_overlay_keys_share_native_path_identity", child=5)
