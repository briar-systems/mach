...
static bool emit_rel32_placeholder(X86Emitter *ctx, const char *label)
{
    ...
    return backend_reloc_list_add(&ctx->result->relocs, BACKEND_SECTION_TEXT, offset, BACKEND_RELOC_X86_64_PC32, label, -4);
}
...
static bool emit_instruction(X86Emitter *ctx, MirInstruction *inst)
{
    ...
    case MIR_OP_ARCH_SYSCALL:
        // emit os-specific syscall instruction
        if (ctx->target->os && ctx->target->os->syscall_len > 0)
        {
            for (uint8_t i = 0; i < ctx->target->os->syscall_len; i++)
            {
                codebuf_emit_byte(buf, ctx->target->os->syscall_opcode[i]);
            }
        }
        break;
    case MIR_OP_RET:
...
{
    static TargetArch arch = {
        .kind         = TARGET_ARCH_KIND_X86_64,
        .name        = "x86_64",
