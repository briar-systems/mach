...
// find a method by searching for symbols that end with ".methodName"
// this handles methods stored with qualified names like "TypeName.methodName"
Symbol *symbol_table_lookup_method_local(SymbolTable *table, const char *method_name)
{
    if (!table || !method_name)
    {
        return NULL;
    }
    ...
    for (Symbol *sym = table->symbols; sym; sym = sym->next)
    {
        ...
        // check if this symbol is a method (name contains '.')
        const char *dot = strrchr(sym->name, '.');
        if (dot)
        {
            // check if the part after the dot matches the method name
            if (strcmp(dot + 1, method_name) == 0)
            {
                return sym;
            }
        }
...
// find all methods with a given name suffix, returning each one in sequence
// call with *start_sym = NULL to begin, then pass the returned symbol's next to continue
Symbol *symbol_table_lookup_method_next(SymbolTable *table, const char *method_name, Symbol *after)
{
    ...
    Symbol *start = after ? after->next : table->symbols;

    for (Symbol *sym = start; sym; sym = sym->next)
    {
        if (!sym->name)
        {
            continue;
