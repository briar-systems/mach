...
static int   config_add_dep_and_save(ProjectConfig *config, const char *config_path, ConfigDep *dep);
...
static int cmd_dep_add(int argc, char **argv)
{
    ...
    // create new DepSpec
    ConfigDep *new_dep = calloc(1, sizeof(ConfigDep));
...
static int config_remove_dep_and_save(ProjectConfig *config, const char *config_path, const char *dep_name)
{
    ...
    // free the dep
    ConfigDep *dep = config->deps[found];
    if (dep)
    {
        free(dep->name);
        free(dep->type);
