...
- A program that takes the address of a function a shared library provides links on aarch64 and riscv64 linux, and on aarch64 darwin. x86-64 takes such an address with the same pc-relative relocation as a call, which the linker already bound to the import's call stub, but aarch64 builds it from an `adrp`+`add` pair and riscv64 from `auipc`+`addi`, which the linker bound to nothing, so handing an `ext fun` to C as a callback failed with `import 'X' pinned to library 'probe' not among the link's dependencies; this link provides: probe`. Both parts of the pair now resolve to the import's PLT entry or stub, as on x86-64, and a relocation against an import that the loader still cannot bind names the relocation instead of blaming the library pin (#3891).
- A darwin program with more than 2 GB of zero-fill links on x86-64 when it calls a shared library, and on arm64 the zero-fill no longer counts against the stubs' 4 GB reach. The Mach-O writer placed the import GOT in a `__GOT` segment of its own after everything the linker had laid out, `__bss` included, so a stub's `jmp *disp32(%rip)` or `adrp`+`ldr`, and a `GOTPCREL` or `GOT_LOAD_PAGE21` data load, crossed the whole zero-fill, and a `var BIG: [3000000000]u8` failed with `macho: a stub's GOT slot lies outside the rip-relative +-2GB range`. The linker now counts the imports that need a slot before it gives anything an address and reserves them as `__DATA_CONST,__got` at the end of the read-only data, ahead of the zero-fill, the way Apple's `ld` lays it out, after any slots it synthesized for defined symbols. A format declares the table through the new `OfVTable.got_shape` and reads its address from `DynamicInfo.got`; the image has one segment fewer. Initialized data still lies between the code and the GOT, and the writer's reach check still refuses a slot beyond it (#3903).
