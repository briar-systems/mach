...
### Fixed
- On x86-64 a divide or widening multiply holds `rax` and `rdx` only at its own instruction. Any function with one reserved both registers from its first instruction to its last, so every other value there took a third register and the pair's results were copied out and back: `(a::u128 * b::u128 >> 64)::u64` returned as `mulq %rsi; movq %rdx, %rcx; movq %rcx, %rax` and a full `u128` product as four moves after the `mulq`. The pair now closes both registers to a value live across the instruction or read by it (the divide writes `rdx` before it reads the divisor), and after it only until its results are read back, so the high half returns as `mulq %rsi; movq %rdx, %rax` and the full product as `mulq %rsi` alone. A packed float divide, which shares the divide's opcode, no longer reserves them at all. 89 x86-64 goldens move across Linux, macOS and Windows, each with fewer instructions and no function longer, and the x86-64 differential agrees throughout (#3760).
