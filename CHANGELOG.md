...
### Added
- Instruction-set extensions. Each instruction set declares its extension names: `ssse3`, `sse41`, `sha` and `fsgsbase` on x86_64, `sha2` on aarch64, and the RISC-V letters and `zicsr`/`zifencei`. Every name is a mach identifier and never a keyword. A target selects extensions with `[target.X].extensions = ["sha"]`, which is refused for a name its instruction set does not declare and joins a RISC-V `isa` string's letters. A level is a bundle: each name may imply others (`sse41` brings `ssse3`; on riscv `d` brings `f` brings `zicsr`) and the selection is closed over that once, at target resolution, so nothing is ever gated on a level name. `$mach.build.extensions.<name>` answers `1` or `0` for the selected isa's own names and is a compile error for any other, never a silent `0`, so a multi-isa source nests it under `$mach.build.arch`. `#[extensions(name, ...)]` marks an outlier function whose inline `asm` may use those extensions; a name another isa declares is accepted and admits nothing, a row only a target selects (riscv `i`, `c`, `f`, `d`) is refused with the reason, the caller owns the run-time check, as with Rust's `#[target_feature]`, and one predicate serves the encoder and the inliner: an instruction requiring an extension is emitted only into a function whose admitted set (the target's selection plus the decorator's) holds it, so an outlier the target already selects may inline into a baseline caller while one it does not never does. `{target.isa}` is the manifest's `isa` value as written and never carries the list. Inline `asm` gains `pshufb` and `palignr` (ssse3), `pblendw`, `pinsrd`, `pextrd` and `ptest` (sse41), `sha256rnds2`, `sha256msg1` and `sha256msg2` (sha), and the baseline `cpuid` on x86_64, where `rdfsbase`, `rdgsbase`, `wrfsbase` and `wrgsbase` now need `fsgsbase`, and `sha256h`, `sha256h2`, `sha256su0` and `sha256su1` (sha2) on aarch64. An extension instruction the target does not select and the function does not admit is refused with the manifest line and the decorator that would admit it (#3521).
