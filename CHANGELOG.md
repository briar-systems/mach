...
- The declared `extensions` set reaches packed-op lowering. A packed-form row carries the extension bits its instruction needs, the lookup behind lowering and the `simd = "require"` judgement takes a gated row only when the selected target holds every bit, and registration checks the bits against the isa's vocabulary. x86-64 gains the first gated row: `i32x4 * i32x4` lowers to `pmulld` under `extensions = ["sse41"]` and keeps the scalar expansion on the SSE2 baseline, so a kernel `simd = "require"` refused on the baseline is accepted once the target declares `sse41`. `pmulld` is spelled in inline asm under the same gate. Legality and shape are unchanged: only the lowering moves with the declaration, and a build never infers the build machine's features (#3737).
