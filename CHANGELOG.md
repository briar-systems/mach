...
- x86-64 code keeps more values in registers. The allocator held `r8`, `r9` and `r10` back for reloading stack-kept values, leaving 10 of the 16 registers for values. Only `r10`, which is already a scratch register, is held back now, so 12 registers carry values. A reload that needs a second register borrows one that no value holds at that instruction. Before any code is written, the allocator checks that every instruction has the registers its reloads need. Where one does not, it moves the value with the lowest loop-weighted use density that is live there, and not read or written by that instruction, to the stack. In a release build of the x25519 bit loop, instructions per bit fell from 73 to 68 in the secret and public forms and from 68 to 61 in the flat form, and stack loads fell from 12 to 8 and 9 to 6. std's portable SHA-256 round loop fell from 74 to 73 instructions, with stack loads down from 4 to 2 and stack stores from 3 to 1. mach-crypto v0.21.0's `encrypt_blocks` keeps its block counter in a register instead of a stack slot, and its frame shrinks from 0x110 to 0xf0 bytes. An AES-128 block runs 13,469 instructions instead of 13,653, and a 1,200-byte AES-128-GCM seal 858.2 k instead of 862.8 k. Leaf functions that needed a callee-saved register before now often use `r8` or `r9` and need no prologue. Of the 402 x86-64 goldens that changed, none grew. aarch64, riscv and SPIR-V output is unchanged (#3495).
...
- `asm spirv { }` is refused where it is written, with `inline assembly is not available on instruction set 'spirv': it declares no assembly capability`, instead of reaching the SPIR-V emitter and failing there as `instruction is not yet supported by the SPIR-V target`. A whole-module instruction set now declares whether it takes inline assembly, as a native one does, and SPIR-V declares that it does not. Behind it, a whole-module emitter fills the object image codegen hands it with its own sections, symbols and relocations instead of returning one byte buffer that codegen wrapped in a section it named `.spirv` itself, so codegen names no target. SPIR-V output is byte-identical (#3370).

### Removed
- The vector literal folds of #3753 and #3738. A literal whose lanes were consecutive loads through one pointer, `f32x4{p[i], p[i + 1], p[i + 2], p[i + 3]}`, was rewritten into one vector load, and a literal of the extensions of one vector's half, `i32x4{v[4]::i32, .., v[7]::i32}` over an `i16x8`, into one lane-halving widen. The range forms of #3779 spell both directly, `p[i, 4]::f32x4` and `v[4, 4]::i32x4`, so a literal is now always built lane by lane. No program changes meaning, and one that still spells a load or a widen as a literal keeps working on the slower lane path. On a target with no packed widen (riscv32, riscv64, spirv) a half range converted by `::` now extends each lane straight from the source vector instead of first copying the half into a slot of its own. `test/cases/vec/load_literal` and `widen_half` moved to the range spelling. Against the literal goldens before the removal, `load_literal` is byte-identical on every column. `widen_half` is byte-identical on aarch64 and spirv, and each probe is shorter on x86-64, where the dead spill of the source the fold left is gone (12 or 13 instructions down to 5 or 6 on linux and darwin, 17 or 18 down to 15 or 16 on windows), and on riscv32 and riscv64 (149 down to 142 for a signed i8 half, 105 to 102 for a signed i16 half, and one fewer for an i32 half). `vec/range` gets the same riscv improvement, 175 down to 142 for a signed i8 half (#3780).

### Fixed
- A `u128` or `i128` argument after an odd number of integer arguments reaches C correctly on aarch64-darwin. mach applied the AAPCS64 rule there, which rounds a 16-byte aligned argument up to an even register pair, so `c_odd(first: i64, v: u128, next: i64)` passed `v` in `x2:x3` and `next` in `x4` where Apple's arm64 ABI starts the pair in the next register, `x1:x2`, and `next` in `x3`, and C read the wrong values in both call directions. The darwin operating system now declares the largest alignment its argument registers honor (8 on aarch64), and the classifier rounds to an even pair only for an argument aligned to 16 after that cap. A 128-bit value on the stack keeps its 16-byte alignment, a pair that no longer fits still gives the last register to nothing, and returns, variadic calls and aarch64-linux are unchanged, each checked against clang for every position (#3922).
- A `u8`, `i8`, `u16` or `i16` argument reaches C correctly on aarch64-darwin. Apple's arm64 ABI makes the caller sign- or zero-extend an argument narrower than 32 bits, and Apple clang's callees read the whole 32-bit register, but mach only truncated, so bits 8 to 31 (or 16 to 31) held whatever the computation left there. The darwin operating system now declares that its caller widens a narrow integer argument to 32 bits on aarch64, a function signature records each integer parameter's signedness below 64 bits, and the call extends the argument by it (`uxtb`, `sxtb`, `uxth`, `sxth`) before it goes in its register. A narrow argument on the stack stays packed at its natural size, and aarch64-linux, x86-64 and riscv are unchanged, each checked against clang (#3927).
- A record or HFA argument that goes to the stack on aarch64 no longer lets a later argument of the same register bank take a register. AAPCS64 gives the rest of the bank to nothing once a composite does not fit the general registers left (C.13) or an HFA or HVA does not fit the vector registers left (C.3), and C reads every later argument of that bank from the stack. mach put `n` in `x7` after a 12-byte record in `e_s(a, b, c, d, e, f, g: i64, s: S12, n: i64)` and in `d6` after a three-double HFA in `e_h(a, b, c, d, e, f: f64, h: H3, n: f64)`, where C read `sp+16` and `sp+24`, in both call directions. A slot now declares the vector registers it consumes beside the general ones, and the classifier exhausts the bank on either spill. aarch64-linux and aarch64-darwin are fixed alike and checked against clang (#3928).
- A record whose members give it 16-byte alignment, such as `rec W { v: u128; }`, reaches C correctly on aarch64-linux. AAPCS64 rounds the next general register up to even for a composite of alignment 16 (C.12), as it does for a 16-byte integer, but mach never rounded a record, so `e_w(a: i64, w: W, n: i64)` passed `w` in `x1:x2` and `n` in `x3` where C read `x2:x3` and `x4`, in both call directions. The alignment AAPCS64 means is the natural one, from the members, so a record raised to 16 by `#[align(16)]` alone is not rounded, and on aarch64-linux it also goes to an 8-byte aligned stack slot, where mach used 16. The aapcs64 convention now declares that an aggregate argument is placed by its natural alignment, darwin declares that it keeps the declared one, as Apple clang does, and aarch64-darwin is unchanged. Each position is checked against clang on both systems (#3929).
- An empty record argument takes no register on aarch64. mach gave `rec E { }` a general register, as if it were a record of 8 bytes or less, so `e_e(a: i64, e: E, n: i64)` passed `n` in `x2` where C read `x1`, and on aarch64-darwin a variadic empty record took a stack slot. clang passes a C empty struct in nothing on aarch64-linux, and Apple's arm64 ABI lets a function ignore one, so the aapcs64 classifier now gives it no register and no stack slot, in both call directions. C++ gives an empty struct size 1 and passes it in a register on linux, and mach follows C. Each position is checked against clang on both systems (#3930).
...
- A union analysis, as the editor and a multi-target `mach check` run, reads a constant that several `$if` arms declare differently from the arm the build target selects. The load bound the constants of every arm some target tuple took into the build target's frame, so `$if ($mach.build.os == $mach.os.linux) { val PAGE: u64 = 4096; } $or { val PAGE: u64 = 16384; }` resolved `PAGE` to whichever arm was walked last, and a `$error` or `$if` reading it could fire for the wrong target. The same held for a constant an arm imports with `use`. Each arm is now walked in a frame that selects it: the build target's when it does, else a tuple's that took it. Single-target builds are unchanged (#3873).
...
- A constant index past 2^31 - 1 into a static array reaches its byte on every target. The offset from the symbol travelled from codegen to the relocation as a 32-bit value, so `BIG[2199999999] = 41` into a `var BIG: [2200000000]u8` failed to link on aarch64 with `relocation 'page21' to 'BIG' overflows`, and on x86-64 and riscv64 linked a store 2 GB below the array. The offset is now 64 bits from codegen to the object formats, which already held it so. It is folded into the pc-relative reference to the symbol only within +-16 MB on x86-64, aarch64 and riscv, since the address the relocation forms must stay in the instruction's reach (+-2 GB for x86-64's disp32 and riscv's `auipc`, +-4 GB for aarch64's `adrp`) wherever the linker puts the symbol, and a larger offset is added to the symbol's address. A constant displacement past disp32 from a pointer on x86-64, which was truncated the same way, is added to the pointer first, and the encoder refuses one it is ever handed instead of truncating it. The COFF writer and the Mach-O x86-64 writer refuse an addend that does not fit the 32-bit field they store it in instead of keeping its low bits. `test/link/cases/3907-far-constant-offset` links constant offsets at, below and past 2^31 into a 2.2 GB array and runs them on x86-64, aarch64 and riscv64 (#3907).
- An `#[oblivious]` function that inlines an inline-asm loop is no longer refused for secrets it holds in registers the loop never reads. The constant-time walk over the emitted instructions kept every register's taint once set inside an expansion that branched locally, and joined the state at a forward skip into every later instruction of the expansion, so a caller's secret in a register the asm later reloaded from a public address or length reached the loop's compares: mach-std#925 had to keep `crypto.ct.zeroize` out of line after sha512's `snapshot_sha384` shape was refused at the kernel's `jae` (inlined, site 5). The assembler now records where each inline-asm local branch lands on its instruction note, and the walk joins the branch's state into exactly that instruction, so every write is a strong update and a register is secret at a compare only when a path to it carries a secret. A local branch whose landing is not recorded reaches every instruction of its expansion, before and after it, instead of freezing the taint, and a skip no longer carries its state past the end of its expansion or loses it past a `ret` inside it. A loop whose compare reads a secret, directly or carried around the loop, is still refused (#3901).
- `mach dep update` re-pins a version-selected dependency to the release its range selects in one run, reading the pin from the recorded gitlink instead of the checkout. When the checkout had drifted from its gitlink to the release resolution picked, update took it as current, and the closure pull that followed restored the old gitlink and printed `(restored to the recorded pin)` with exit 0. With the range raised past the pin (`^7.0` to `^8.0`), that left a pin outside the range, and with an in-range drift it needed a second run to update. The locks that keep every other identity where it is and `mach dep outdated`'s pinned column read the recorded gitlink too. `mach dep list` now shows a pin its range excludes as `out of range (the pinned release 7.0.2 is outside ^8.0)` instead of `realized` (#3878).
- `mach dep add` with no `--version` names the version conflict when no release fits, instead of an error from an old release's manifest. Resolution walks back through older releases, and a tag whose `mach.toml` predates the current schema stopped it with that manifest's error, printed as `error: mach.toml: unknown key 'name' in [project]` as if the project's own manifest were wrong. A release whose manifest does not load, or names another version, is now no candidate: resolution sets it aside and picks the highest release that fits past it, and when none does the error lists the conflicting requirements (`gl 0.4.0 requires std ^6.0`, `root requires std ^8.0.0`) with each set-aside release as `gl 0.1.0 is not a candidate: its mach.toml does not load: unknown key 'name' in [project]` (#3881).
...
- A darwin program with more than 128 MB of static data links on aarch64. The Mach-O writer placed its call stubs in a `__STUBS` segment after everything the linker had laid out, zero-fill included, so a `bl` from the code to a stub crossed the whole `__bss` and a `var BIG: [140000000]u8` failed with `macho: dynamic call-site displacement overflows a 26-bit branch`. The linker now reserves the stub table as a `__TEXT,__stubs` section at the end of the code before it gives anything an address, the way Apple's `ld` lays it out, so a call reaches its stub whatever data the image carries, on x86-64 as on aarch64. A format declares the table through the new `OfVTable.stub_shape` and reads its address from `DynamicInfo.stubs`; ELF and COFF still place their own stubs. The GOT keeps its own segment after the data, and a stub whose slot lies beyond its reach (+-4 GB for the aarch64 `adrp`, +-2 GB for the x86-64 `jmp *disp32(%rip)`) is now refused at link time instead of written with a truncated displacement (#3888).
...
### Changed
- A parenthesized name or literal spans its parentheses like every other expression, completing #3720: `(x)`, `(42)` and `("s")` no longer keep their bare token span, so a diagnostic about the whole grouped value (a type mismatch, a cast fix) starts at the `(`. The seven token-backed kinds carry their token in their own payload (`tok` on `ExprIdent`, `ExprLitInt`, `ExprLitFloat`, `ExprLitChar`, `ExprLitStr` and `ExprLitNil`), and every reader of a name or a literal's text takes it from `tok`: interning a name, rescanning a literal, reading a string's content (`expr.lit_str_content`), a comptime gate, a decorator argument, a debug variable's name. A diagnostic about the name itself (an unresolved name and its spelling fix, a deprecated or `#[testing]` use) still points at the name. `abitype.literal_is` refuses any node that is not a string literal before reading it, `expr.span_is_token` is gone since nothing asks it any more, and `Expr.data.lit_char` is a record whose code point is `.value`. Code built by the compiler is unchanged (#3747).
...
- A module-scope `$if` that reads a constant declared through a `def` is decided when the module loads. The loader follows the `def` chain across `use` and `fwd` to the integer primitive at its end, so the constant keeps that width and signedness. It used to leave such a gate untyped and decide it only after type checking, which std v7.4.0 did for 91 gates in its test build. A chain that ends outside the integers, such as a record or a pointer, is refused at the gate with an error naming the chain. A `def` declared inside a `$if` arm is read in the arm that arm's gate selects, decided first (#3790) (`doc/language/comptime-control.md`) (#3791).
- A module-scope `$if` with an arm that declares something is decided while the modules load, in one pass that reads its constants in the order they depend on each other rather than the order they are written in. Such a gate reads build facts (`$mach.*`, `$bin.*`, `$project.*`) and `val` constants from any module: one declared later in its own module, one declared under a later `$if`, whose gate is then decided first, or one a later `use` imports. A comptime cast in it converts to the integer its destination names, directly or through a `def`. Gates and constants that lead back to each other are refused at the gate with an error naming the cycle, as `` `$if (A == 4)` -> `$if (B == 4)` -> `$if (A == 4)` ``. Such a gate may not ask a type question, and one it asks through a constant it reads is refused like one it asks itself, naming the question and the constants it goes through: `` `$size_of` asks a type question through `T` -> `S` ``. A chain whose every arm declares nothing is decided during type checking as before and may still measure a type. A gate the load cannot decide is refused where it stands, saying what it cannot evaluate, and rejects the load the way an unresolvable `use` does instead of failing it. A union analysis decides each chain in every target tuple's own frame, so a constant or module name that an arm binds differently per tuple is read per tuple. The multi-round machinery that used to reach these answers is gone: deferred gates and the resolve rounds between them, the per-module gate typing pass and its product, the per-round load views and the union tuple gate pass. Code built by the compiler is unchanged (`doc/language/comptime-control.md`) (#3790).

### Added
- Range indexing `x[start, count]` with a comptime `count`. On a vector it reads `count` lanes as a vector, and on an array or a pointer it reads a `[count]T` value, a copy and never a view. A range is an assignment target whose stored value has exactly the range's type. A constant start on an array or a vector is bounds-checked (`start + count == N` is in bounds), and a pointer range is unchecked. `p[i, 4]` is a range and `f[T, U]` stays type arguments: the resolver decides `v[a, b]` by what the name is. `::` converts between `[N]T` and `TxN` of the same element type and count, while `:~` stays the bit reinterpret. An array in memory converted to a vector is one vector load, and the reverse is one vector store, with no slot and no lane insert. `v[0, 4]::i32x4` and `v[4, 4]::i32x4` over an `i16x8` lower to the lane-halving widen on x86-64 (baseline and sse41) and aarch64, signed and unsigned. aarch64 gains the six `sxtl`/`uxtl` and `sxtl2`/`uxtl2` rows for it, so the literal fusion of #3738 fires there too. A module `val` folds a range over a constant array or vector and the array/vector `::`. `doc/language/types.md`, `operators.md` and `grammar.md` describe the form, and `test/cases/vec/range` pins it against a C reference on every column (#3779).
- `$type_id(T)` gives each type a `u64` identity (#3861). It is distinct for distinct types: two records of one name in two modules differ, `^T` differs from `T`, and composites and generic instances differ by their parts. It is equal for one type in every module, at every instantiation site and in every compilation unit of one program, since it is derived from the type alone: the first 64 bits of SHA-256 over the type's canonical recipe, never an address or a session's numbering. It folds as a compile-time constant wherever the layout intrinsics do. A collision is a hash collision, at most `n(n-1)/2^65` for `n` types, and a unit with no view of the whole program cannot rule it out, so `doc/language/comptime-intrinsics.md` states that bound beside the contract. The value is promised within one compiler version, not across versions.
...
- Vector shifts return as `vec << vec` and `vec >> vec` over one integer vector type (#3740, reversing #2030). Each lane shifts by the count in the same lane exactly as the scalar operator does: `>>` is arithmetic on signed lanes and logical on unsigned ones, a lane count at or above the lane width saturates (`0`, or the sign fill), and a constant lane count at or above the width, or negative, is a compile error, as #3756 made it for scalars. There is no vector-by-scalar shift: `v << 3` is a located type error that names the form to write, `u32x4{c, c, c, c}`. A count that is the same value in every lane of a literal (one constant, or one value such as `u32x4{n, n, n, n}`) shifts every lane by that one scalar, the form every baseline set has an instruction for: on x86-64 it is one `psllw`/`pslld`/`psllq`, `psrlw`/`psrld`/`psrlq` or `psraw`/`psrad` by an immediate, or by the count moved into an xmm register with `movd`/`movq`, and those instructions saturate on their own, so the scalar shift's range test is not needed. 8-bit lanes have no packed shift on SSE2 and shift as 16-bit lanes with the neighbouring byte cleared; the 64-bit arithmetic shift (`psraq` is AVX-512VL) and every per-lane count (`vpsllv*` is AVX2, #3751) take the scalar expansion, one saturating scalar shift per lane. aarch64 and SPIR-V shift lane by lane for now: NEON's `sshl`/`ushl` read a count's low byte as signed and do not saturate past it, and `OpShift*` is undefined at or above the width. `simd = "require"` judges a shift by its count's form, so a per-lane shift is refused on x86-64 and a uniform one is accepted. `doc/language/operators.md` has the rule, the realization table and a runnable example; `vec/rows_i{8,16,32,64}` gain shift rows and `vec/shift_splat` pins the #3736 hashing idiom against a C reference.

### Fixed
- `::` between an array and a vector follows the vector rule. It used to fall through to the nonnumeric rule, so an equal-size pair such as `[8]i16::i32x4` silently reinterpreted its bytes. It now converts element by element. It needs the same count, as two vectors already did, and the same element type. A mismatched pair such as `[8]i16::i32x4` is now an error that names the differing count, as a mismatched vector pair already was, and a byte reinterpret is spelled `:~` (#3779).
- Checking a pinned dependency against a requirer's version range, as `mach dep verify` does, no longer frees a string literal when the pinned commit carries no release tag. The release lookup returned the literal `""` in that case and an allocated string otherwise, and the range check freed either one, so on a build where the literal started a page the free unmapped it and the next read of `""` crashed. The lookup now returns an allocated string on every path, and so does the lookup of the release a dependency's committed gitlink names, which had the same mixed contract. Every reader of either, including `mach dep update`, `mach dep outdated` and the resolver's locks, frees what it receives (#3865).
- `mach init` scaffolds a binary whose source passes `mach fmt --check`, and names its entry `src/main.mach` (`entry = "main.mach"`) instead of `src/root.mach`. The template listed `use std.runtime;` before `use std.print;` and aliased `print:`, which repeats the path's last segment, so a fresh project failed `mach fmt --check .` until it was formatted. An init test now formats every scaffold template and requires it unchanged. `--lib` still writes `src/lib.mach`, and existing projects are unaffected, since the compiler attaches no meaning to the entry's basename (#3817).
- A release page carries its notes. The release job published archives with an empty body, so v5.11.0 had to be filled by hand. `.github/scripts/release-notes.sh <tag>` now writes the body from the tag: the version's `CHANGELOG.md` section, then `## Issues resolved (N)` listing each issue a pull request merged since the previous tag resolved, whether it links the issue, names it on a `Closes` line, or is named by the comment that closed the issue by hand. An issue closed before the pull request naming it was opened is left out. CD publishes the release with that body and fails when the script fails, and running it on v5.11.0 reproduces the hand-written body byte for byte (#3828).
- The dependency surface no longer differs from the manifest in silence. `mach dep pull`, `mach dep update` and `mach dep add` print one note on stderr, whatever `--quiet` says, for each requirement a root `ref` or `path` declaration overrides, naming the identity, the root's selection, the requirer chain and what it asked for (`note: dependency 'std': the root declares ref = "tag/v2.0.0", overriding hedgeacme -> hedge -> std which requires ref = "tag/v2.1.0"; ...`), the note `mach dep verify` has printed since #3548. Only a root selection over a chosen release's range was noted before, so a root pin older than a `ref` or `path` dependency's was invisible. `mach dep list` shows each root declaration's winning selection, now `version=` too, and under it every requirement it overrides. `mach dep verify` and every build refuse a gitlink outside the root's own `tag/` selection, naming `mach dep update <path> <id>`: the root's `tag/` was never checked against its pin, so a gitlink left at another release built green against it. A checkout that left its recorded gitlink is refused naming `mach dep pull` (restore) and `mach dep update` (re-pin), and an unrealized dependency, including the empty directory a fresh clone leaves for an uninitialized gitlink, is refused as `is not realized` naming `mach dep pull` instead of `must be the repository root`. A build still fetches and realizes nothing. `mach dep pull` and `mach dep update` say whether each path dependency's copy was refreshed from its source or reused, and `update` syncs a path copy once instead of twice (#3455).
- `mach test` tests one artifact's closure, the way `mach build` builds it. A test build loaded every artifact's entry and then every module under `src`, so a module no artifact reaches was type-checked and its tests run under `mach test` while `mach build` and `mach check` never saw it, and every module read the default artifact's `$bin.name`, so a `$if ($bin.name != "shaded") { $error(...) }` that `mach build --bin shaded` accepts failed `mach test .` (#3778). `mach test` now selects the artifact as before (`--bin`/`--lib`, the sole candidate, or the one marked `default = true`) and loads, checks and runs that artifact's closure only, with that artifact's `need`, steps and `link` entries, so `$bin.name` in a test block is the artifact under test. Tests in modules no artifact reaches no longer run: give such modules a library artifact whose entry `use`s them and test it with `--lib` (`doc/language/test.md`). The compiler does this itself: `[artifact.tests]` (entry `src/lib/tests.mach`) reaches the driver, sema and codegen suites the compiler never imports, CI runs `mach test . --lib tests` beside `mach test .`, and the two runs together list exactly the tests one whole-source run listed. With no `--bin`/`--lib`, `mach build` and `mach check` now read the same default selection as `mach test`: the artifacts marked `default = true` among those the target builds, and every one of them only when none is marked. A project that marks a default no longer builds or checks its other artifacts without a selector, so `mach build .` in mach builds only the compiler again and `-o` works (`doc/language/manifest.md`, Selection and the build matrix). The `note: skipped N target-gated modules` line, which only the whole-source load could produce, is gone (#3813).
...
- A variable rebound to the value an instruction just defined is located from that instruction's end. The binding was published at the start of the next instruction the lowering emitted, so anything later placed between the two left the variable homeless for that stretch: in `test/link/cases/debugger-gdb`'s release image the loop counter `i` of `accumulate` had no location for the `movq %rdx, -0x1040(%rbp)` that stores `total` between `addq $0x1, %rcx` and the loop test. A binding of the def just lowered now opens at that instruction's end, an expansion's end is its last piece's end, and a spilled def's end is the end of the store that puts the value in its slot, so the rebinding opens at the same offset as the record that ends the earlier location (#3688).
- On x86-64 a divide or widening multiply holds `rax` and `rdx` only at its own instruction. Any function with one reserved both registers from its first instruction to its last, so every other value there took a third register and the pair's results were copied out and back: `(a::u128 * b::u128 >> 64)::u64` returned as `mulq %rsi; movq %rdx, %rcx; movq %rcx, %rax` and the full `u128` product of two `u64` with four moves after the `mulq`. The pair now closes both registers to a value live across the instruction or read by it (the divide writes `rdx` before it reads the divisor), and after it only until its results are read back, so the high half returns as `movq %rdi, %rax; mulq %rsi; movq %rdx, %rax` and the full product as `movq %rdi, %rax; mulq %rsi` with nothing after it. A packed float divide, which shares the divide's opcode, no longer reserves them at all. 89 x86-64 goldens move across Linux, macOS and Windows, each with fewer instructions and no function longer, and the x86-64 differential agrees throughout. This completes the x86-64 half of #3691 (#3760).
...
### Added
- A widen of a value the machine already holds extended is a move. A sub-word load leaves its general register zero-filled (`lbu`/`lhu`, `movzx`, `ldrb`/`ldrh`), a `zext` or `sext` fills to its width and an immediate move fills to its width, yet the `zext` or `sext` that widened such a value to the register or lane width converted it again: `movzbq %dl, %rsi` after `movzbl (%r8,%rdx), %esi`, `slli`/`srli` after `lhu`, `uxth` after `ldrh`. A pass over the shared machine IR, before width legalization and selection, records per virtual register the fill every definition leaves (its origin and reach, met over every definition and carried through copies, so a phi whose edges are a loaded byte and a small immediate is known too) and rewrites a `zext` or `sext` that reads at or above the fill's origin and writes within its reach into a move the allocator coalesces; one that writes past a whole-register fill on a 32-bit register reads the whole register instead, so the low lane of a 64-bit widen on riscv32 is a move again and the o2 goldens that grew under #3616 shrink back. The rule is one fact about the value, not per-isa pattern matching, and a sign extension of a load stays until a target loads sign-extended (#3660).
- A shared library build warns at a `fwd` of a generic, comptime-parameter or pack declaration. Such a `fwd` in a root-project module exported nothing and said nothing: the declaration has instances rather than one definition, each mangled with its arguments and materialized where it is used, so there is no symbol for the export surface. The lowering now knows the artifact it feeds is a shared library and warns once per `fwd`, naming the declaration and its kind and pointing at a `pub` non-generic wrapper around the wanted instantiation as the way to export one. A `fwd` of a module that holds generics warns for none of them, since it asked for the module's exportable surface, and the same lines in an executable or static library build raise nothing. `doc/language/visibility.md` states the rule (#3439).
- A vector literal whose lanes are consecutive scalar loads from one pointer at stride one lowers to one vector load. `f32x4{p[i], p[i + 1], p[i + 2], p[i + 3]}` went through the stack slot every literal takes on a machine target, four scalar loads, four lane stores and a vector load of the slot; it is now the load the pointer reinterpret `@((?p[i]):~*f32x4)` spells, `movups (%rdi,%rsi,4)` on x86-64, and the two release texts are byte-identical at every lane type. The optimizer recognizes the slot the front end lowers the literal to, every use one lane store and one whole load, each stored value a non-volatile load of the lane type through one base at the index `c + k` for lane `k` (a constant, a value, or `add value, constant`, over a scalar or an array pointee), and nothing between the first lane load and the whole load that writes memory or calls; each lane load is read by its store alone, so the rewrite drops it with the slot. The rule is target-independent, since a vector's bytes are its lanes packed at every width, so an `f32x8` literal folds too and the target lowers the one load as it does any vector load. A permuted order, a stride, a second pointer, a constant lane, a volatile load or an aliasing store between the lanes keeps the lane path. `test/cases/vec/load_literal` pins the probes against a C reference on every column that expresses pointer arithmetic (#3753).
- `doc/language/secrecy.md` states the constant-time multiply per instruction set. A new section carries the declared rows as a table, one line per instruction set and multiply naming the widths and the condition (x86-64 every scalar cell always, aarch64 the low half at every width, the 64-bit high halves and the 32-bit widening products under PSTATE.DIT, riscv64 the low half at every width and the three 64-bit high halves with `m` and `zkt` selected, riscv32 and SPIR-V none), the three condition kinds, the realized cells (the 64-bit widening product on aarch64 and riscv64, the 128-bit product everywhere), the evidence behind each row with the kind of claim marked, why DOITM is not a multiply condition and has no manifest key, and the fail-closed rule: an undeclared cell is a compile error, never a slower substitute, and a `PSTATE.DIT` binary on a processor without the mode is refused at start. A unit test reads the table back against the registered rows so the two cannot drift. `operators.md` states `*` on `^T`, `manifest.md` that `extensions` is the manifest's only lever over the secret multiply and that no key declares or overrides a timing mode, and `policy.md` the compiler and stdlib halves of the fail-closed rule (#3510).
- A manifest's `extensions` list spells the x86-64 microarchitecture levels: `x86-64-v2`, `x86-64-v3` and `x86-64-v4` are accepted beside the extension names and each stands for its published member set, so `extensions = ["x86-64-v3"]` selects everything v3 requires (v2 plus `avx`, `avx2`, `bmi1`, `bmi2`, `fma`, `lzcnt`, `movbe`, `f16c`) and a name listed beside a level adds to it. A level is a bundle of an isa's extension bits, not a bit of its own: `$mach.build.extensions.<name>`, `#[extensions(...)]`, asm `requires:` and packed-form gates keep answering per extension, an asm block gated on `sse42` assembles under `x86-64-v2`, and a level is refused where an identifier is required. x86-64's vocabulary grows by the thirteen names the levels need (`sse42`, `cx16`, `avx`, `avx2`, `bmi2`, `fma`, `movbe`, `f16c`, `avx512f`, `avx512bw`, `avx512cd`, `avx512dq`, `avx512vl`), each carrying its `implies` chain, so the vocabulary is 20 names. A manifest that spells a name outside the vocabulary is refused with both lists, `its extensions are: ...; its levels are: ...`. Registration checks a level table: hyphenated spellings that no row answers, members inside the vocabulary and each level a strict superset of the one before, and a test reads the level table in `doc/language/manifest.md` back against the compiler's so the two cannot drift. `native` is not a level and nothing infers the build machine's features (#3741).
- A vector literal that takes the same-sign extension of one narrower vector's consecutive lanes lowers to the lane-halving widen. `i32x4{v[0]::i32, v[1]::i32, v[2]::i32, v[3]::i32}` over an `i16x8` went through the stack slot every literal takes on a machine target, eight extracts and four stores for one half; it is now one `vec_widen_s` (or `vec_widen_u` for a zero extension) of the source's low half, and lanes 4..7 its high half. The optimizer recognizes the slot the front end lowers the literal to, every use one lane store and one whole load, each stored value the extension of the source's lane at the literal's position, and rewrites only where the target's catalog packs the cell, so a target without a packed row keeps the lane path unchanged. x86-64 declares the six rows (8 to 16, 16 to 32 and 32 to 64 bits, signed and unsigned): the SSE2 baseline is `pxor`, `pcmpgt` for the sign and `punpckl`/`punpckh`, and under `extensions = ["sse41"]` it is `pmovsx`/`pmovzx`, with a `psrldq` for the high half. The ten instructions are spelled in inline asm too, byte-exact against gas, and `doc/language/asm.md` lists them. `test/cases/vec/widen_half` pins the twelve probes against a C reference on every column (#3738).
- The declared `extensions` set reaches packed-op lowering. A packed-form row carries the extension bits its instruction needs, the lookup behind lowering and the `simd = "require"` judgement takes a gated row only when the selected target holds every bit, and registration checks the bits against the isa's vocabulary. x86-64's first gated choice is `i32x4 * i32x4`: `pmulld` under `extensions = ["sse41"]`, the `pmuludq` sequence on the SSE2 baseline (#3739). `pmulld` is spelled in inline asm under the same gate. Legality and shape are unchanged: only the lowering moves with the declaration, and a build never infers the build machine's features (#3737).
...
- A link case can exercise a test build. `goal: test` in `case.conf` makes the case's compile step `mach test` in place of `mach build`, with the same target, profile and flags and the leg's qemu engine passed as `--runner`, so every module of the project is loaded and the collected tests run as part of the step; under `run: exec` the recorded observable is each collected test's stdout through the dispatcher, in collection order. `test/link/cases/3523-orphan-ext-after-layout-gate` is the first such case: an orphan module whose `ext` follows a gate only layout can decide, which `mach build` never loads and so could not cover the #3523 defect, now links and runs on every leg, the darwin ones included (#3535).
- `i32x4 * i32x4` and `i64x2 * i64x2` lower to packed sequences on the SSE2 baseline instead of the scalar expansion. The 32-bit multiply is the two-`pmuludq` interleave (`pshufd` the odd lanes of both sides, multiply the even and the odd pairs, `pshufd` each product's low dwords back together, `punpckldq`), which `pmulld` replaces as one instruction under `extensions = ["sse41"]`; the 64-bit multiply is the three-`pmuludq` schoolbook (`psrlq 32` each side, the two cross products added and shifted up by `psllq 32`, plus the low product). Both cells move from the scalar row to the packed row in x86-64's catalog, so a kernel `simd = "require"` refused on the baseline for either multiply is accepted, and the `pmulld` gate now chooses between two packed lowerings rather than between packed and scalar. `psrlq` and `psllq` immediate forms join the encoder; they are encoder-internal like the other immediate shifts and have no inline-asm spelling. `vpmullq` under avx512dq is #3749 (#3739).
- A driver-level `-g` test reads `.debug_loclists` back from the object image on x86_64, aarch64 and riscv64 for a function whose first parameter's register a temporary takes before a call and whose second parameter's caller-saved register the call takes: each of the three lists the function owns (found through the relocation on its base address) is one register offset pair, the clobbered parameter's ends at the clobber before the call's target field and the other two end at the instruction after the call (found through the call relocation), then end-of-list. Disabling the encoder's clobber recording turns it red. This is the end-to-end test #3641 deferred while `driver/tests.mach` was being rewritten (#3680).

### Fixed
- A returned or passed value reaches its fixed register from the operand that dies in it, without a scratch copy. `(a::u128 * b::u128 >> 64)::u64` returned on aarch64 as `umulh x2, x0, x1; mov x0, x2` and on riscv64 as `mulhu a2, a0, a1; mv a0, a2`: a value whose only reader is a copy into a fixed register (a return, an argument) refused that register when its current holder died at the very instruction defining the value, although the two never overlap. The holder now hands the register over, as a copy source already did, and both are the one instruction. Handing over exposed a second heuristic gap: a value live across a call that found no free callee-saved register took a caller-saved one and was then spilled whole by the post-pass, where evicting a sparser callee-saved holder is cheaper, so with holes the allocator now compares densities there as it does under full pressure. The x86-64 form of the same return (`mulq; movq %rdx, %rcx; movq %rcx, %rax`) has a different cause, the function-wide reservation of `rax` and `rdx` around any widening multiply or divide, and is deferred to its own issue. The corpus goldens move on every target and the x86-64 differential agrees throughout (#3691).
- A by-reference aggregate parameter has a `DW_AT_location` under `-g`. On aapcs64, lp64d and win64 the caller passes a record wider than the register pair as the address of a copy it owns, and the callee's home for the parameter is that incoming pointer, moved into a callee-saved register when the body keeps it live across a call; the DWARF producer rejected a register home for an aggregate outright, so on exactly the conventions that pass the object by address the parameter had no location at all and a debugger showed `w=<optimized out>`. sysv64's memory-class case, a 32-byte record the callee addresses through a register too, was missing the same way. The location now describes the memory the pointer names: `DW_OP_breg<n> <net>` while the pointer is in a register, `DW_OP_fbreg <slot>, DW_OP_deref` once it is spilled, the class gcc and clang emit for the same parameter. The register home is as bounded as a scalar's, since the encoder ends a register location at the instruction that clobbers it, which is what the rejection guarded against when it was written. `test/link/cases/3461-byref-param-location` reads the class back through llvm-dwarfdump on every ELF leg, and its `-win64` sibling on the COFF image (#3461).
- A shift count at or above the operand's width saturates. `x << n`, `x >> n` on an unsigned `x` and every `<<` answer `0` when `n` is at least the width of `x`, and `>>` on a signed `x` answers the sign fill (`0` or `-1`); the count may be any integer type, including one wider than the operand. Until now the count was whatever the machine made of it, its low 5, 6 or 7 bits (`u32 << 40` was `<< 8`), and a `u8` or `u16` shift, computed at 32 bits, kept the bits above the type until the consumer narrowed them, so the answer at counts 8..31 was right and at 40 was `<< 8` again. The saturation is one unsigned compare of the count against the width, and for a logical shift a mask of the result by its negation, for an arithmetic shift the count or'd with the flag less one, so it is branch-free, admits the same secret counts the constant-time gates admitted, and is applied once in MIR lowering, so no isa grows an instruction. It wraps the legalized 128-bit barrel the same way. A count the IR already bounds below the width, an in-range constant or a value masked by a constant under the width (`x << (n & 63)`, seen through the extends and truncations that carry it to the count type), carries no saturation, so its code is unchanged. A count that is a compile-time constant at or above the width, or negative, is a compile error, `shift count 40 is at least the width of \`u32\` (32 bits)`, at sema through a literal, a `val`, an expression of them or a comptime path, and in `$`-evaluation against the operand's declared width; the constant folder folds such a count to the saturated value where inlining or propagation exposes one later instead of leaving the shift unfolded. `doc/language/operators.md` states the rule. The corpus pins it per width and signedness: `test/cases/bits/shift_{u,i}{8,16,32,64}` and `test/cases/int128/shift_{u,i}128`, each with opaque counts at `w-1`, `w`, `w+1`, `2w`, `2w+7` and `255` (and `33` at 8 and 16 bits, where the 32-bit ALU would wrap the count to 1) and a seeded count straddling the width, against C references that saturate, and `test/cases/ct/shift_secret` shifts by a secret count in `#[oblivious]` functions, which the validator refuses to build around a branch; the four existing 32- and 64-bit cases, which pinned the masking and called it defined, are rewritten. Vector shifts follow with the same rule in #3740 (#3756).
- RISC-V inline asm jumps to a symbol. `j sym`, `jal sym` and `jal rd, sym` were refused (`inline-asm branch/jump target must be a numeric local label`) where `b sym` on aarch64 and `jmp sym` on x86-64 assemble, so a body written once for the three ISAs failed to parse on RISC-V alone. The jal rows now take a symbol the way the aarch64 grammar does (a numeric local first, then a symbol region), emitting one J-type word under a new `RK_JAL20` relocation kind that the ELF writer spells `R_RISCV_JAL` and reads back, the linker applies with the 20-bit even-displacement check (`+-1 MiB`, refused past it), and a jump to an imported function patches to its PLT stub as one J-type word rather than the `auipc`+`jalr` pair a `call` site is, which the PLT fixup now records the kind of. A conditional branch to a symbol is refused at the asm site (there is no 12-bit branch relocation kind), a malformed local (`1x`) is named as one, and the constant-time scan classifies a direct symbol transfer exactly as the numeric-label form. `doc/language/asm.md` has the section (#3243).
- A relocatable object round trip keeps hidden visibility and export requests on every format. The COFF emitter dropped `SYM_OBJ_FLAG_HIDDEN`, since COFF has no visibility bit, and no emitter wrote the image's `fwd` export requests, so an object written and parsed back exported every global and forgot every request, and a `dll` built from parsed objects exported its internals. COFF now writes a `.drectve` section of ` /EXPORT:<name>` tokens, one per exported definition and one per request, and the parser reads the directives back: every defined global is hidden unless a directive names it, a directive naming no definition is a request, `-export:` and `/EXPORT:` in any case with a `,DATA` suffix or quotes are read as link.exe reads them, and an object with no `.drectve` is left as it was. ELF and Mach-O keep their native visibility spellings and carry the request list in a non-loaded mach section, `.mach.exports` and `__MACH,__mach_exports`, that the parser consumes to a zero-length husk the linker skips, and refuses when malformed. The object image is not mutated on emit: the section rides in a view. `doc/language/manifest.md` states the per-format encodings (#3423).
- `$bin.name` under union analysis is the artifact whose entry reached the module. The editor session analyzes the project as the union of every artifact (#3774), but the artifact name was one value per cell handed to every module's comptime context, so a module of `shaded` analyzed in a session whose primary artifact is `hello` read `$bin.name == "hello"` and a `$if ($bin.name != "shaded") { $error(...) }` that `mach build --bin shaded` accepts was reported. The union walk now records on each module the artifact whose walk first reached it, the primary artifact's when the primary's walk reaches it and otherwise the first artifact's in manifest order (a module first reached from inside the deferral loop is its importer's artifact's), and the module's comptime context reads `$bin.name` from that attribution, through an in-place refresh of the module's text too. The attribution joins the module's load view, the per-module input its resolve, gate and sema products are keyed by, so a module another artifact comes to reach first is typed again rather than replayed. A build of one artifact and a test build still attribute every module to that artifact, and their output is byte-identical. `doc/language/comptime.md` states the rule (#3776).
- An editor session analyzes a project as the union of every artifact. The retained analysis entry (`analyze_project_tolerant`, which the language server uses) derived its root set from the request goal alone, the selected artifact's closure or the test build's whole project, so every open file was checked inside the primary artifact's cell: a file of a non-default artifact was held to the wrong artifact's `need`, and `#[embed("{artifact.<id>.out}")]` in it was refused with `names an artifact this artifact does not require` although `mach build --bin <that artifact>` accepts it. The analysis entries now take the root set (`project.ROOT_ARTIFACT`, `project.ROOT_UNION` or `project.ROOT_TEST`) beside the request, and a union cell resolves `{artifact.<id>.out}` over every artifact some artifact needs, as a test build already did, so `shaded.mach` embedding the shader output `shaded` needs resolves in a session rooted at `hello` while an output nobody needs is still refused. The editor session asks for the union, which also selects the default artifact of a multi-artifact manifest where the artifact cell refused to load without one named. The root set and the requirement scopes join the build configuration fingerprint, so a module's resolve and sema products are not reused across cells that resolve different requirements. `$bin.name` is still the selected artifact's name for every module of the union (#3774).
- An `ext val` on aarch64-darwin links. The aarch64 lowering reads imported data through the GOT, an `adrp`/`ldr` pair carrying the GOT relocation kinds, and the Mach-O object writer had no spelling for them, so every program that read an `ext val` on aarch64-darwin failed with `RelocKind got_page21 is unsupported by macho` (aarch64-linux was wired in #2079). The pair is now written as `ARM64_RELOC_GOT_LOAD_PAGE21` and `ARM64_RELOC_GOT_LOAD_PAGEOFF12` and read back as the GOT kinds, symbol-based and only over the `adrp` and 64-bit `ldr` a GOT load is made of; a GOT slot has no addend, so the writer refuses one and the reader refuses an `ARM64_RELOC_ADDEND` in front. A cell defined in the image is reached through the linker-owned slot in `__DATA_CONST,__got`, rebased under `--pie`, and a cell a dylib provides through the `__GOT` slot dyld binds, which the image writer now patches for the pair as it did for the x86-64 forms. `test/link/cases/macho-got-aarch64` cross-builds the three reads on the linux leg and inspects the object's records, the slots and the bind, and executes the image on a native arm64 macOS leg (#3675).
- `#[inline]` expands at every direct call site regardless of the caller's expansion budget. The attribute overrode the size and use-count heuristics and then lost to the per-caller 1024-instruction budget downstream, silently: with a body of about 200 statements the call stayed a call and no diagnostic said so, and because the budget was spent in call order an ordinary small callee expanded earlier in the same caller could consume what a later forced expansion needed, so whether the attribute held depended on what else the caller called. A forced expansion is no longer charged against the budget, so it never loses to the heuristic expansions around it and the outcome does not depend on call order; the budget now bounds heuristic expansion alone. The correctness refusals are unchanged: a `noinline`, `naked` or `scalar` callee, a recursive cycle, an indirect call and an `oblivious` body outside an `oblivious` caller stay calls. `doc/language/decorators.md` states the contract (#3425).
- `doc/language/types.md` says how a vector is loaded from memory. The only idiom it showed was the full-arity literal, which for `f32x4{p[i], p[i + 1], p[i + 2], p[i + 3]}` is four scalar loads, four lane stores into a stack slot and a vector load of the slot; the load idiom is the pointer reinterpret, `@((?A[i]):~*f32x4)`, one unaligned vector load (`movups` on x86-64) at any element offset, and the store is the same cast on the destination. The page also states what a shape wider than the vector register costs on the 128-bit seed: `f32x8 + f32x8` on x86-64 is one scalar operation per lane through a stack slot, counted by the `simd = "scalarize"` note and refused under `"require"` (#3755, #3736).
- A parenthesized expression's span covers the parentheses: `parse_paren` now widens the returned node's span to the paren extent instead of returning the inner node's span, so a diagnostic on `(a + b) * c` starts at the `(` and an editor selection range steps from the inner expression to the parenthesized group. Only the token-backed leaves (identifiers, comptime identifiers and the int, float, char, string and nil literals) keep their token span, since their readers take the text through it. The formatter reads a grouped unary's operator and a grouped literal's body through its own tokens again, where the wider span had spelled `(- x)` and unmarked `([2]i32{1, 2})`, and a named entry whose value starts with a paren now takes part in the entry alignment its neighbours already had (#3720).
- An editor session no longer reloads the whole project closure for every edit. Any text change marked the loaded project stale, so each keystroke the language server analyzed rebuilt the module set from the entry, re-walked every module's uses and gates, and re-derived resolve and sema across the closure: 335 modules and 750 ms on the compiler's own tree, against the 3 ms the edited module's own work costs. An edit is now the edited module's alone until the load walk says otherwise: the session records the edited buffers and the next analysis reparses and re-walks each one in place, keeps the project when the module still reaches the same modules with the same public constants, gate outcome and target gating, and reruns the query phases over it so every unchanged module's resolve and sema are hits and only the edited module and its dependents recompute. An edit that changes the load surface (a `use` of a module the closure did not hold, one dropped, a gate that decides differently) falls back to the closure load, as does a newly opened buffer, which is a new analysis root. Under the debug test binary the compiler's tree loads in 37 s and refreshes in 155 ms per edit. The module's old load diagnostics go with its old tree, so a diagnostic fixed by the edit does not survive it (#3719).
...
- `test/run.sh` runs the differential whether or not the golden matched. It returned on a golden mismatch before the C-reference comparison, so a change that moved the disassembly silently skipped the behaviour check and `N FAIL, all golden` read as the behaviour having passed when it never ran. One `FAIL` line now carries the golden verdict and then either `differential agrees with the C reference` or the disagreement, a case whose differential never executed is counted in the summary as `differential not run`, and no such case counts as a pass. The C reference answer was already rebuilt when its source is newer (#3459, #3452).
- The `enqueue_instance` scaling test no longer decides from wall-clock time. It timed 1500 and 6000 enqueues and failed when the large one took more than eight times the small, so a scheduling stall in the half-millisecond small measurement went red under machine load with nothing wrong. It now counts the allocator calls and bytes the same populations cost through a counting allocator: the instance vector and the name index both grow geometrically, so quadrupling the population adds a fixed handful of calls and stays within the doubling envelope of bytes, and a linear growth policy adds thousands of calls and squares the bytes. The counts print on pass as well as on failure. On a quiet machine the old measurement gave 372 us against 1087 us, 2.9x for a 4x population (#3444).
...
- riscv32 builds an f64 program that reaches for `0.0` or passes an f64 past the `fa` registers. Two defects. A float literal is an immediate the encoder materializes, and its zero and one-word shapes went through `fmv.d.x`, which is RV64-only: an f64 `0.0` on rv32 now spells `fcvt.d.w fd, zero` and every other f64 pattern loads from the constant pool, while f32 and rv64 output is unchanged. And ilp32d follows the integer convention for an f64 past `fa7`, a register pair (`a1:a2`), one register with the high word on the stack (`a7` and `0(sp)`), or the stack, which the lowering carried as one register move, a cross-bank move rv32 has no spelling for; a float in general registers on a target with no cross-bank move of its width now goes through a frame slot, its bits joined from or picked into the register pieces exactly as a u64 argument's lanes are, on the argument, parameter, return and call-result sides alike. `float/arith_f64`, `float/special_f64`, `generic/operators`, `vec/cast_lanes`, `vec/cast_lanes2` and `call/call_float_regs` build for riscv32 and agree with the C reference under qemu-riscv32, and leave the column's `SKIPS`. The build summary now counts a failure record that reached no diagnostic store (an encoder or rules refusal) in its tally, so `1 error / 1 warning` prints above exit 2 where `0 errors / 1 warning` did (#3617).
- The optimizer's two program refusals, a vector operator whose lane shape the target's catalog does not declare and an operation that would scalarize under `simd = "require"`, are located errors on the module's diagnostic store, not text on a bare failure. They travelled outside the store, so the build summary's tally did not count them and `0 errors` could print beside a nonzero exit. Each now appends one `error` at the operation (or the function's declaration when the operation has no location) and answers `reported`, the way the codegen refusals do, so the rendered outcome ends `1 error / 0 warnings` and the exit code is the user's. The parallel lowering workers report into their prepared query's store, so a refusal from a worker thread reaches the presentation like one made on the main thread (#3664).
...
### Fixed
- `mach dep update` and `mach dep verify` agree when two closure dependencies name one transitive identity by range. The closure walk kept the first request it met for an identity and dropped every later release-selected request whose url agreed, so the resolver's problem carried the first declarer's range alone: with `b` requiring `x >=1.0` and `c` requiring `x <1.2`, update chose the highest `x` in `>=1.0`, and verify, which checks every manifest, refused the pin `c` never admitted. The retained request now carries every further range with its requirer, one need each, and the root's own range narrows the same way, so resolution intersects every requirer's range and a conflict names each requirer by its chain (`root -> c requires x <1.2`). `doc/language/manifest.md` states the rule (#3702).
- `mach test -o <path>` places the test dispatcher at the path. The flag was accepted and never read: the plan derived the dispatcher's path from the `out` template alone, so two `mach test` runs in one tree rewrote and ran the same binary and one of them failed with `text file busy`. The override now names the one binary a cell links, a built artifact under `mach build` or the dispatcher under `mach test` alike, resolved against the project root and refused outside it as `build -o` already was; a required artifact keeps its declared output, and `--list` still links nothing. Runs with different `-o` in one tree link different files; they still share the object tree under `out/` (#3701).
- The width-legalize pass folds a sign extension of an immediate. `expand_sext` emitted the runtime sign-fill sequence for every source, so a `MIR_SEXT` whose source is an immediate of at most 64 bits widened a value known whole at legalization at run time, on riscv32 and for the 128-bit lanes on 64-bit targets. Such a widen is now a lane-wide immediate move per destination lane, the high lanes the sign, and its expansion count says so. No program reaches the fold today: the IR constant folder folds every sign extension of a constant at every optimization level and the MIR lowering of a promotion folds its own immediate, so no golden changes; the legalizer honours the MIR contract that admits the operand, pinned by unit tests at each source width (#3700).
...
- A generic's type arguments reach its callee by declaration, not by position. An instance replaced every type parameter it met by the argument at that parameter's index, whichever declaration the parameter belonged to, so a caller whose type-argument list was reordered or longer than its callee's handed the callee the wrong types: `find[K, V, M, P]` calling `hk[K, M, P]` had hk's body checked against `(K, V, M)`, a `$type_of` gate selected the wrong arm, and a field in what should have been the dead arm was refused (`no such field`), while every caller whose parameters happened to sit at the callee's positions worked. A type substituted once was also mapped again on every later read. Substitution now carries the declaration whose parameters it replaces and leaves a parameter of any other declaration alone, so a second pass is the identity; every scope that substitutes (an instance check, a call with explicit type arguments, the union-secrecy walk, lowering) names its owner from the one derivation that also interns the parameters. `find_extra` compiles, a concrete instance through two generic layers lowers the arm its policy names, and a concrete instance whose arm reads a field the type lacks is still refused with the instantiation trail (#3711).
- A generic called from another generic's body with that generic's own type parameters as arguments is no longer judged as an instance. Such an open instance was recorded whenever the callee's body carried a type directive and its body was then checked loudly against bare parameters, so a field of a forwarded type parameter inside a `$type_of` gate (`$type_of(h.order)` with `h: *X`) was refused with `no such field` although the template pass had rightly said nothing, and the two templates alone, with no call site, failed to compile. An open instance is the template under other names: nothing in its body is settled until the caller is itself instantiated, and that caller's instantiation records the closed instance, which is judged as before, with the trail. Every closed instance is recorded; the filters that chose among closed instances were already overridden and are gone (#3710).


## [5.9.0] - 2026-09-19

### Added
- `u128` and `i128`. The two 128-bit integers are primitives like the others: every arithmetic, bitwise, shift, comparison and conversion operator applies, literals take the `u128` and `i128` suffixes and are scanned, range-checked and folded at 128 bits, comptime evaluates them exactly, `$size_of` is 16 and `$align_of` 16, and they cross an `ext fun` boundary as C's `__int128` does on each convention: two registers under System V, LP64 and AAPCS64 (with AAPCS64's even-register rounding and register exhaustion, C.10 and C.11), and by reference to a caller copy with an `XMM0` return on Win64, the GCC and Clang convention that LLVM 18 aligned on, verified against a clang probe. No target has a 128-bit register, so a value is realized as two 64-bit lanes by the width legalizer on x86-64, aarch64 and riscv64, which declare the width in their machine model; riscv32 and SPIR-V declare no such integer and refuse the type at the declaration or expression that names it. A multiply whose operands are both the same extension of one narrower type to twice its width is recognized as the widening product and compiles to the target's one widening instruction, so `((a::u128) * (b::u128)) >> 64` on `u64` limbs is a single `mul` on x86-64, `umulh` on aarch64 and `mulhu` on riscv64, with no 128-bit product materialized; the plain 128-bit product is the schoolbook over the lanes, and each target declares how it forms a 64-bit full product (the three-address high multiply on aarch64 and riscv64, the fixed `rdx:rax` pair on x86-64). Division and remainder at 128 bits call helpers the compiler provides with the program, one per operation, coalesced across modules by weak linkage. A secret 128-bit multiply is admitted exactly when every 64-bit cell of its realization is (`$mach.build.ct_mul(low, 128)` reads the answer), so the secret limb product of a multi-precision library compiles to that one instruction; a secret 128-bit division is refused like every secret division. The vector lane rule is now stated: a lane is an integer or float of 8 to 64 bits, so `u128x2` is refused by name (#3511).

### Fixed
- The loop analysis reads a counted loop's step at the induction variable's width. A negative step narrower than 64 bits (an `i32` step of `-1`) was read as its zero-filled 64-bit pattern, a large positive number, so the loop's direction was misjudged; the step now has the sign it spells, and an induction variable wider than 64 bits is not counted, since the trip-count arithmetic is 64-bit.
- A `#[handle(...)]` operand above one 32-bit word is refused with `must fit one 32-bit word` instead of being truncated to its low word.
- The register allocator holds a call's return registers until the instructions that read them. A value that died on the copy into an argument register could take that register while it still carried the other half of the previous call's two-register result, so on aarch64 and riscv64 a 128-bit value returned by one call and passed one register over to the next arrived with its high word replaced by its low word (`mv a1, a0` followed by `mv a2, a1`). A physical register read after a call is now reserved from the call to the position before the read, the mirror of the window an argument register is busy over before its call, so the copy lands elsewhere and both words arrive (#3690).

## [5.8.1] - 2026-09-19

### Fixed
- `mach dep pull <subproject>` realizes the gitlink the enclosing repository commits under the subproject. A git dependency's record (its staged gitlink, `.gitmodules` entry and module directory) was read only when the project root was the repository root, so a subproject's dependency was always a plain clone: on a fresh clone of a repository with a subproject declaring `version = "^5.7"` and a committed `test/consumer/dep/std`, `pull test/consumer` cloned std at its default branch and then failed with `checkout of '': dependency ref must be branch/<name>, tag/<name> or commit/<id>`. The record now has a place, the repository's top level and the gitlink's path from there, the subproject's prefix ahead of `dep/<id>`, and a subproject whose dependency has a gitlink takes the root's own path: `pull` initializes or restores the gitlink's commit, `update` stages the moved gitlink, and a range with neither a gitlink nor a checkout is refused as unpinned. A subproject with no gitlink still gets a plain clone. `doc/language/manifest.md` and the `mach dep` reference state the contract (#3686).
- `mach dep add` and `mach dep update` resolve the version ranges every dependency of the closure declares, not the root's alone. A range declared by a dependency reached by a `ref` or a `path` never entered the resolution, so it arrived unpinned and `pull` refused it with `has no pin yet; run mach dep update <root> <id>`, a command that failed the same way; only a range reached through another range worked, which is why std, declared by every root, hid the gap. Resolution now takes its needs from the same closure walk `pull` realizes, over every edge kind: a range a ref or path dependency declares is a need, an identity such a dependency selects by ref or path is fixed and a requirement on it overridden (the note names the declaring project or chain), and a manifest reached through a release is left to the resolver as before. A range with no checkout under the root yet is seeded by the committed gitlink of the dependency declaring it, mapped to its release, so `add` and a single-name `update` land where the dependency pinned, while `update --all` still moves to the highest release every range admits. To read the manifest being selected, `update` advances the fixed selectors before resolving and `add` realizes a ref or path dependency before resolving. The hinted command now succeeds for a transitive range (#3689).

## [5.8.0] - 2026-09-19

### Added
- aarch64-linux and aarch64-darwin admit the secret integer multiply. An operating system now declares, per instruction set and behind one accessor, whether it guarantees the processor's data-independent-timing mode a `DIT_MODE` constant-time row needs; linux declares it for aarch64 (`HWCAP_DIT` in `AT_HWCAP` since v4.17, PSTATE preserved across every exception) and darwin declares it for aarch64 (Apple's "Enable DIT for constant-time cryptographic operations": per-thread state set with `msr dit`, checked through `hw.optional.arm.FEAT_DIT`), each with its citation beside the declaration; windows and freestanding declare nothing, so a secret multiply there is refused with a diagnostic naming the instruction set's DIT rows and the operating system. The rows only hold while the mode is on, so the compiler records the program's need and the runtime honours it: a module whose lowering admits a secret multiply through a DIT row carries a local absolute symbol `__mach_needs_dit` (value 1) in its object, which survives ELF, Mach-O, COFF, archives and the build cache and reaches no linked image, and an executable linked for a declaring target gets one synthetic input defining `__mach_dit_required`, a hidden one-byte read-only object that is 1 when any input carries the mark and 0 otherwise. The std start code reads the byte before `main`, turns the mode on when it is set (and at every thread it creates) or fails closed with a fixed message and exit status 255 when the processor or kernel lacks it, and never touches DIT when it is 0. The mark is per module, so a dead-stripped multiply still sets the byte, and "a binary without the need carries nothing" holds up to that one zero byte; a shared library gets no cell. `$mach.build.ct_mul` answers 1 for the aarch64 rows exactly where the OS declares the guarantee. The corpus case `ct/mul_secret` now builds on the aarch64-linux and aarch64-darwin columns and runs end to end on the native aarch64-linux leg. `doc/language/secrecy.md` documents the declarations, the symbol contract and the fail-closed path (#3508).
- The aarch64 inline assembler spells the three architectural barriers and the speculation barrier: `dsb <option>` with the full option set (`sy`, `st`, `ld`, `ish`, `ishst`, `ishld`, `nsh`, `nshst`, `nshld`, `osh`, `oshst`, `oshld`), `isb` bare or as `isb sy`, and `sb`, which is an extension instruction under the new `sb` extension (FEAT_SB) and refused, naming the extension, on a target that does not select it and in a function without `#[extensions(sb)]`. `msr dit, 1` already assembled; the immediate of a PSTATE write is spelled bare, since `#` opens a comment in an `asm` block, and `doc/language/asm.md` now says so. None of the four reads or writes a register, so an `#[oblivious]` body may spell the runtime's DIT sequence, `msr dit, 1; dsb nsh; isb`, beside a loaded secret (#3508).

### Fixed
- On a 32-bit target, a `-g` link with a function that dead-code elimination collected succeeds. The linker wrote the DWARF dead-code tombstone as the 64-bit `-1` whatever the width of the field, so on riscv32 the `abs32` code address of every collected function overflowed and the link was refused, whether or not `_start` was present. The tombstone is now all ones at the width of the field (`0xFFFFFFFF` for a 32-bit code address); a dead line-table sequence still starts at 0, and 64-bit targets write the same bytes as before (#3677).
- Linking an executable whose entry symbol is undefined reports the missing entry before any relocation is applied. The check ran only when the image was written, after relocations, so a riscv32 `-g` build with no `_start` failed with `relocation 'abs32' overflows` from a debug section's code address instead of the `undefined entry symbol '_start'` the plain build gives (#3642).
...
- A variable's DWARF location range ends where a later instruction writes its register, whether that instruction defines another variable or a compiler temporary. A range was bounded only when another variable took over the storage, so a parameter whose register was reused by a temporary after its last use kept a location naming that register for the rest of the function, and a debugger read the temporary as the parameter; the same held for a rebinding that left the variable with no home. The encoder now watches the register each emitted instruction defines, and treats a call or an inline asm block as defining every register the ABI's declared callee-saved set does not keep, records the loss of every location published on those registers by an earlier instruction, and the DWARF emitter treats such a record as a bound, so a variable with one home becomes a location list rather than a whole-function register location. Observed on riscv32 under the qemu-system gdbstub as a `u64` parameter reading stale after its lane register was reused (#3641).
...
### Changed
- The x86-64 register allocator models where a value is dead. A value's live range is now the union of the positions where it is read, written or live across, so a register is free across a hole, and a value dead across a call does not hold its register there. The loop-end copies that rotate a loop's carried values are the main beneficiary: each rotated value is dead between its last read and the copy that renews it, so the copies now write registers other values just left. When registers run out the victim is the value with the lowest loop-weighted use density, on either side of the decision, so a value read inside the hottest loop is never the one evicted. A spilled value gets a second chance: a block-local run of two or more of its reads that would each reload is carried by one load into a fresh value that competes for a register like any other, and a load that would itself spill is withdrawn. A target declares whether its allocator models holes (`lifetime_holes`); x86-64 does and the other targets do not, so their output is unchanged byte for byte. Every spill and reload is still placed before the code runs, so constant-time code is unchanged in kind. In a release build of the x25519 bit loop, instructions per bit fell from 79 to 73 (secret form), 87 to 73 (public) and 82 to 68 (flat), with stack stores down from 14 to 9, 16 to 9 and 16 to 9 and register copies down from 18 to 7 and 17 to 8 in the public and flat forms. std's SHA-256 round loop fell from 106 to 72 instructions, with stack loads down from 13 to 4 and stack stores from 28 to 3; across the whole function the state pointer, which the density rule now keeps on the stack in favour of the round variables, is loaded once per run of reads instead of once per read, so the function's stack loads fell from 18 to 11 (#3495).
...
- On x86-64, `x = a - x` in a loop computed 0: the register allocator let the result of a non-commutative two-address operation take the register of its right operand when that operand was the value the result was then copied into, and `sub d, r` runs as `mov d, l; sub d, r`, so the copy overwrote the right operand before it was read. The same shape reached shifts by a register count and a right operand read through memory. The allocator now grants that register only when the operation reads the value before it writes, which a two-address operation does for its left operand and, by swapping, for the right operand of `+`, `*`, `&`, `|` and `^`. The x86-64 encoder refuses a right operand living in the destination register as an internal error instead of emitting the clobber. aarch64 and riscv64 were never affected because their operations are three-address (#3647).
- A variable wider than the target's registers keeps its location in `-g` output. When the width pass splits a value into lanes, each debug binding on it becomes one binding per lane, and the DWARF emitter describes the variable as a composite of `DW_OP_piece` entries, one register or frame-slot piece per lane in memory order, with an empty piece for a lane whose home is unknown. Before, the pass discarded the bindings of every instruction it expanded, so on riscv32 no `u64` local or parameter had a location at all. A lane counts as a home of its own when the allocator reuses storage, so another variable taking one lane's register bounds the composite, and a composite taking a variable's register bounds that variable. A copy the allocator deletes now hands its bindings to the instruction that follows it, where it used to drop them, so a variable whose binding rode a coalesced copy (the first piece of a shift by exactly one lane, for instance) is located too. Read back with gdb through qemu, a riscv32 `u64` local prints its value in both of its homes (#3603).
...
### Added
- Instruction-set extensions. Each instruction set declares its extension names: `ssse3`, `sse41`, `sha` and `fsgsbase` on x86_64, `sha2` on aarch64, and the RISC-V letters and `zicsr`/`zifencei`/`zkt`. Every name is a mach identifier and never a keyword. A target selects extensions with `[target.X].extensions = ["sha"]`, which is refused for a name its instruction set does not declare and joins a RISC-V `isa` string's letters. A level is a bundle: each name may imply others (`sse41` brings `ssse3`; on riscv `d` brings `f` brings `zicsr`) and the selection is closed over that once, at target resolution, so nothing is ever gated on a level name. `$mach.build.extensions.<name>` answers `1` or `0` for the selected isa's own names and is a compile error for any other, never a silent `0`, so a multi-isa source nests it under `$mach.build.arch`. `#[extensions(name, ...)]` marks an outlier function whose inline `asm` may use those extensions; a name another isa declares is accepted and admits nothing, a row only a target selects (riscv `i`, `c`, `f`, `d`, `zkt`) is refused with the reason, the caller owns the run-time check, as with Rust's `#[target_feature]`, and one predicate serves the encoder and the inliner: an instruction requiring an extension is emitted only into a function whose admitted set (the target's selection plus the decorator's) holds it, so an outlier the target already selects may inline into a baseline caller while one it does not never does. `{target.isa}` is the manifest's `isa` value as written and never carries the list. Inline `asm` gains `pshufb` and `palignr` (ssse3), `pblendw`, `pinsrd`, `pextrd` and `ptest` (sse41), `sha256rnds2`, `sha256msg1` and `sha256msg2` (sha), and the baseline `cpuid` on x86_64, where `rdfsbase`, `rdgsbase`, `wrfsbase` and `wrgsbase` now need `fsgsbase`, and `sha256h`, `sha256h2`, `sha256su0` and `sha256su1` (sha2) on aarch64. An extension instruction the target does not select and the function does not admit is refused with the manifest line and the decorator that would admit it (#3521).
...
- A secret multiply in an `#[oblivious]` function compiles to the hardware multiply on riscv64 when the selected instruction set includes Zkt (`isa = "rv64gc_zkt"`). Zkt is a new selectable extension, spelled `_zkt` after the other `z` extensions, and it lists `mul`, `mulh`, `mulhsu`, `mulhu` and `mulw` as data-independent-timing. The riscv64 rows admit the low half at 8, 16, 32 and 64 bits and the three high halves at 64 bits, and `$mach.build.ct_mul` reports the same cells. Without Zkt or without M the multiply is still refused, and riscv32 declares no rows. x86-64 declares Intel's DOIT multiply list (`mul`, `imul` and `mulx` at every operand size), which holds unconditionally (see the Changed entry for #3508), and aarch64 declares Arm's DIT list (`madd`, `smaddl`, `umaddl`, `smulh`, `umulh`), which holds only under DIT, so a secret multiply is still refused on every aarch64 target until an OS guarantees the mode (#3508). The corpus gains a `riscv64zkt-linux` column that checks the secret multiply against a bit-serial reference under qemu (#3509).
- `test/run.sh --docs` compiles every mach code block in `doc/language`, and runs each one that declares a main, with the compiler under test and the checkout's std. The fence line marks a block that is not a whole program (```` ```mach fragment ````) or one that must fail with a named diagnostic (```` ```mach error <text> ````), and CI runs it on every pull request. Six examples in `secrecy.md` and `ext-fun.md` did not match the compiler and are fixed: the secret add example is `#[oblivious]`, four blocks import the `usize` and `bool` they name, and one comment quotes the diagnostic the compiler gives (#3583).
- `test/run.sh --qemu` runs the riscv32 corpus column's differential under `qemu-riscv32`, the way riscv64's runs under `qemu-riscv64`. A riscv32 case's artifact is a freestanding static archive, so the driver builds a run bin per case from `test/lib/start_riscv32.mach`, whose `_start` reads `argc` and reports the checksum through raw linux syscalls, and `test/lib/elf_loadable.py` re-lays the freestanding image into the one `PT_LOAD` shape qemu-user maps without changing a byte the program sees. Which emulator serves a target is a column of the target table, and a missing one is announced rather than silently dropped. The column's blanket `SKIPS` line, which cited a legalizer refusal that no longer happens, is gone: 108 of 114 cases build and carry a blessed golden, six that refuse the o2 build are named one per line with the refusal, and a new `test/golden/<target>/NORUN` file lists the 62 cases that build but compute a wrong checksum under the emulator, so their goldens still carry the column while the disagreement is counted rather than failed. Neither file is trusted: a `SKIPS` case that builds and a `NORUN` case that agrees with the reference fail the run as stale lines. A column that deliberately serves a subset of the corpus states it in `test/golden/<target>/ONLY` instead, which is a decision about the column rather than a claim about a case, so it is not checked; `riscv64zkt-linux` serves `ct/*` that way. Running the column found three riscv32 codegen defects: an array indexed by a `u64` reads its index from a register nothing assigned (#3615), a `u8` or `u16` widened to `u64` is not masked (#3616), and the f64 lowering reaches an RV64-only encoding (#3617). CI runs the riscv32 goldens on every pull request beside the other ISAs and the qemu differential at the main tier (#3604).
- The identifiers the 128-bit integer work will use are claimed ahead of it, so parallel work cannot take the same numbers: the type kinds `TYPE_U128` and `TYPE_I128` (appended, nothing renumbered), the IR opcodes `OP_MUL_HIGH_S` and `OP_MUL_HIGH_U`, the MIR opcodes `MIR_MUL_HI_S`, `MIR_LANE_JOIN` and `MIR_LANE_PICK`, and two machine-model fields: `int_widths`, the scalar integer widths a target realizes (1, 2, 4 and 8 bytes on every target, read only through `realizes_int_width`), and `mul_wide_form`, how it realizes a full-width product (a three-address high multiply on RISC-V with the M extension, none elsewhere). Both fields are part of the target fingerprint. Nothing spells, accepts or emits a 128-bit type or any of the new operations yet: `u128` is not a type name, and no diagnostic changes. The primitive test everywhere is now the type catalog (`type.is_prim`) instead of a position in the kind numbering, and `resolve` seeds its primitive symbols from the one `type.PRIM_COUNT` instead of a private copy of the count (#3609).

### Changed
- A secret multiply in an `#[oblivious]` function compiles to the hardware multiply on every x86-64 target, on Intel and AMD, with no OS declaration and no manifest key. The x86-64 rows (the low half, both high halves and both widening products at 8, 16, 32 and 64 bits, `mul`, `imul` and `mulx`) now hold unconditionally instead of only under DOITM, and `$mach.build.ct_mul` reports the same cells. Intel's DOIT guidance states that processors which do not enumerate DOITM behave as if it were enabled for the listed instructions, and `mul`, `imul` and `mulx` are on that list. AMD publishes no such list, but BearSSL's per-core `ctmul` table records a constant-time multiply at every width for every AMD core from K7 through Zen, no AMD x86-64 core has been documented with a data-dependent multiplier, and AMD's SB-1039 advises constant-time algorithms. DOITM itself hardens memory-side predictors that the kernel owns and does not touch the multiplier (#3623). The corpus builds `ct/mul_secret` on the x86-64 columns and checks it against the bit-serial reference natively. aarch64 keeps its DIT rows and still refuses the secret multiply until an OS guarantees the mode (#3508).
- A by-value aggregate argument is copied once per call, not twice, when nothing evaluated after it in the same call can write it. The value is still captured where the argument is evaluated. A later argument that holds a call, an assignment or an address keeps the copy made at that point, and anything the analysis does not recognise keeps it too. Otherwise the call's own ABI placement is the only copy, as it already was for a temporary such as `f(make())`. An inlined callee copies such an argument before its body runs, since it has no ABI placement. Secret-bearing and `#[volatile]` records, and SPIR-V, keep the copy. On `test/cases/call/call_rec_mutate` the object shrinks from 1502 to 1184 instructions on x86-64, 1373 to 1138 on aarch64, 3475 to 2338 on riscv64 and 1650 to 1265 on win64 (#3460).
...
- `doc/language/manifest.md` describes `kind = "shared"` as it ships, where it used to call the kind reserved for a later phase. ELF targets write a `.so`. Mach-O `.dylib` and PE `.dll` output are not written yet (#3588). The page covers the export surface, the refusals for a library that exports nothing and for `freestanding` targets (the `raw` format produces only executables, and `of = "elf"` still refuses at the link because the os maps no loader), and how internals are hidden. The 5.3.0 entry for #3412 says hidden definitions are `STV_HIDDEN`. That holds in the per-module objects, but in the linked `.so` they are `LOCAL` symbols absent from `.dynsym`, and no Mach-O or PE library is written at all (#3581).
...
- Packed vector lane conversions on x86-64 and aarch64 now compute directly in the destination register. They fall back to a scratch register only when the destination is a frame slot, or when it is the operand's own register and the conversion still reads the operand after writing the result. Both encoders build their conversion as a step sequence, and that one sequence decides whether the destination may alias the operand. On x86-64 this removes the copy out of scratch after every conversion. The aarch64 output does not change (#3537).
- On aarch64, a frame slot more than 256 bytes below the frame pointer is loaded or stored in one instruction instead of two. It is addressed as a scaled offset from `sp` instead of through `x16` from `x29`. The frame now records whether `sp` stays fixed between the prologue and the epilogue, and that is false for a function whose body writes `sp`, through inline asm or a register operand. A frame where `sp` is not fixed, or where the slot's `sp` offset is not a multiple of the access width (a packed field, say), keeps the two-instruction form. In mach's own aarch64 build, two-instruction frame accesses fell from 155,945 to 3,571 and `.text` shrank from 9,828,924 to 9,219,052 bytes (6.2%) (#3540).

### Fixed
- The missing-compiler-range warning and `mach init` suggest `mach = "^5.3"`, the oldest release that reads `[project].mach`, on every 5.x compiler, and `^N.0` on a later major N. They used to suggest the running compiler's own minor, so the line depended on which binary the author ran, and a project told `^5.4` stopped building on a 5.3.0 CI seed. The warning now says the floor is the project's to raise when it uses a later feature (#3572).
...
- A writable global whose scalar initializer is zero (`var n: usize = 0;`, `var counter: u64;`, or a float whose bits are all zero) is emitted as zero fill (`.bss`, `__bss`, uninitialized data) on ELF, Mach-O and COFF, as a zero aggregate already was, instead of taking file bytes in `.data`. `-0.0`, non-zero scalars, read-only globals and `#[section]` globals keep their placement. An `f32` global whose `f64` initializer underflows to `+0.0` is zero fill too (#3421).
...
- `mach dep add` accepts `--offline`, as `doc/language/manifest.md` and the 5.3.0 notes already said. It resolves from the releases already fetched, prints `resolving from releases already fetched (--offline)`, and refuses an identity it has no releases for by name. An offline resolution that reaches a url with no checkout now says `offline: no local checkout of <url>` for `add`, `update` and `outdated`, where it used to print the raw git failure from a missing `dep/<id>` (#3579).
- `mach help <cmd> <action>` and `mach <cmd> <action> -h` print that action's block: its grammar, effect, exit codes, options and constraints, rendered from the same schema as `mach help dep`. Both were refused as `malformed help request`. An action the command does not have is refused, naming the command's actions. `dep` is the only command with actions (#3597).
- `mach init --no-deps` tells you to run `mach dep update <root> std`, which pins and realizes the declared release. It used to name `mach dep pull <root>`, which refuses a version-selected dependency with no pin. The hint and `pull`'s refusal now build the command from one helper, and a test runs the hinted command (#3580).
...
- `mach dep verify` and every build check version-selected dependencies offline. The pinned commit must carry a release tag, and that release must be inside every requirer's range. A pin outside a range names the requirer chain, the range and the pinned release, and prints the `mach dep update` command that re-pins it. A root `version` narrows the requirers' ranges and doesn't override them, so both are checked. A root `ref` or `path` still overrides. A release, meaning a dependency reached through a `version` range or an exact `tag/`, may itself select dependencies only by `version` or `tag/`. One that selects by a branch, a commit or a path is refused, naming the chain and the offending line. A branch-selected dependency may still select anything. `mach dep verify --release` applies the same rule to the project itself, for a release workflow to run before tagging (#3503).
- A git dependency can select releases by range: `[dep.X] git = "..."` with `version = "^1.2"` instead of `ref`. `mach dep add <path> <name> --git <url>` resolves the highest release the running compiler and every range accept and writes `version = "^X.Y.Z"` of it (or the range given with `--version`). `mach dep update` resolves every version-selected identity in the closure, root and transitive, and checks each slot out at its release; naming one identity keeps the others where they are, `--lowest` selects the lowest release every range accepts, and `--offline` resolves from tags already fetched. `mach dep outdated` reports each identity's pinned, highest compatible and latest release. A release whose manifest selects a dependency by branch, commit or path is refused, since it cannot be reproduced from its tag. A version-selected dependency without a pin is refused by `pull` and every build with the update command to run (#3502).
- Dependency resolution has its resolver: PubGrub over each identity's listed releases, with the running compiler checked per release. It picks the highest release every requirement and the compiler accept (or the lowest, for a minimal-versions check), keeps a locked version while it still fits, and explains a failure by the requirements that prove it, naming the identity the root can declare to decide. Nothing uses it yet; `mach dep add` and `mach dep update` will (#3501).
...
- Each instruction set can now declare which secret-operand multiplies have data-independent timing, one row per cell: the low half, the unsigned, signed or mixed high half, or the unsigned or signed widening product, at an operand width of 8, 16, 32 or 64 bits, each under a condition (always, a data-independent-timing mode the target guarantees, or a selected extension). This replaces the single yes/no switch per instruction set. MIR lowering, the `#[oblivious]` validator and the post-encoding walk each ask for the exact cell they emit, registration refuses a malformed row (a row for a lane multiply is refused as not defined yet), and the target fingerprint covers the rows. No instruction set declares a row yet, so every secret multiply is refused as before. `$mach.build.ct_mul(op, width)` folds to 1 when the selected target admits that cell and to 0 otherwise, from the same decision the gate makes, so a library can pick its hardware or bit-serial path at comptime. `doc/language/comptime-mach.md` documents it (#3507).
### Changed
- The gate pass is a query. Each module's gate decisions, and the surface its importers' gates read, are one product keyed by the module and target, and a union build keeps one per target tuple. A warm rebuild with no gate-relevant change runs no gate pass at all. An edit that leaves a module's gate surface unchanged reruns only that module's gates, and not its importers'. Sema now depends on the gate decisions it reads, which it used to read untracked. A project whose gates need types still loads in more than one round, and every round rebuilds each module's resolve and gate products. mach-lsp's warm rebuild after an unchanged edit fell from about 1.54 s to 0.90 s. A one-shot command has nothing to reuse and pays for recording the gate results: a warm `mach check` of mach-lsp went from 3.47 s to 3.66 s, and a `mach build` of mach from 12.74 s to 12.91 s (#3472).
- aarch64 builds a constant from ones with `movn` when that takes fewer instructions than building it from zeros with `movz`. A small negative number, such as the offset of a frame slot more than 256 bytes below the frame pointer, took four instructions and now takes one. Every such slot access went from five instructions to two. Mach's own aarch64 release build shrank from 12.19 MiB of code to 9.69 MiB, and the `.wt/ctloop` benchmark from 9101 instructions to 8193. `movn` is also accepted in inline `asm` blocks (#3527).
- `doc/language/manifest.md` now documents version compatibility as one contract (#3505):
  - the range grammar, its tables and the pre-1.0 and pre-release rules;
  - `[project].mach` and `[dep.X].version`;
  - what resolution picks, what `add` writes, and `--offline`, `--lowest` and `outdated`;
  - root narrowing against root override, and the rule that a release selects only releases;
  - what verify checks, the no-yank policy, and forks.

  The note that ranges would be a separate decision (#3112) and the `version` key's "reserved for the registry era" wording are gone.
- `mach init` adds std like `mach dep add <path> std --git <url>`: at the caret range of the newest std release whose `[project].mach` admits the running compiler, checked out at that release, instead of tracking a branch. The scaffold's `[project]` states `mach = "^X.Y"` for the compiler that created it. `--no-deps` still resolves and writes the range but skips the checkout, so it needs the network too. Offline it fails, says so and prints the `mach dep add` command to run once connected, and writes no `[dep.std]` table. `mach dep pull <path>` realizes the declared release later (#3504).
- The three checks that walk a build-step path one component at a time share one walker, which takes a policy per call:
  - validate: every component exists, and the last has the expected kind;
  - contained: the walk may stop at a missing component;
  - create: a missing directory is made.

  Each policy refuses a symlinked component or a symlink loop anywhere in the path, and a fix now lands once instead of three times. Diagnostics now name the path prefix where the walk stopped, on every platform. On Windows, only a component that does not exist counts as missing; any other inspection error is reported instead of being treated as absent. A directory that a concurrent build created first is accepted there too, as it already was elsewhere (#3532).
- The `--cache` object store now writes each entry whole to a temporary file and renames it into place, so a reader never sees a partly written entry. Entries were written in place before, and every read hashed its payload with SHA-256 to detect a torn write. Reads now check a fast 64-bit checksum, which is enough against bit rot, and the digest was never a defence against anyone who can write the cache directory anyway. On a warm `--cache` build of mach, restoring the 315 cached objects went from about 1.1 s to 0.55 s, and the `cache` row from 1.5 s to 1.0 s. The entry format changed, so entries written by an earlier compiler read as misses and are replaced. A temporary left by a crashed build is skipped by the store and removed once it is ten minutes old, the next time the store compacts its journal (#3520).
- mach builds against std 4.0.0, pinned at `tag/v4.0.0` (it was `tag/v2.0.0`). CI now seeds from mach 5.2.1, the first release std 4.0.0 builds with (it seeded from 5.0.0). The compiler follows std's new OS contract: descriptors cross it as `usize` handles opened through an out parameter, raw returns are classified with `os.error_kind` instead of compared with `os.E*` constants, and native codes are described with `os.error_message`. A removal beneath the project root now reports its own refusals (a name too long, a path that escapes the root) as failures with a kind. Under std 4.0.0 those refusals carry no native code, and `mach clean` and `mach dep remove` would otherwise have read them as success. Two diagnostics from the build-root ancestor check also no longer read the path component after freeing it, and the "cannot open ancestor" message names that component.
...
- An integer `0 - x` lowers to the target's negate on every ISA, so x86_64 emits `mov x, r; neg r` instead of `mov $0, r; sub x, r`, and aarch64, riscv64 and SPIR-V drop the materialized zero. A floating-point `0.0 - x` is unchanged, since it isn't `-x` when x is +0.0. x86_64 lowers a unary `-x` the same way, where it used to emit `xor r, r; sub x, r`. The reduced x25519 multiply loop from the issue went from 119 to 118 instructions per bit with `^u64` limbs, and from 123 to 116 with public ones. The register allocator also no longer spills its chosen victim when the incoming value can't use the register that frees (#3495).
- With `--cache`, `-v` reports the persistent cache's setup and restore as their own `cache` row before `lower`, and `-vv` lists the `cell snapshot`, `cached object` and `compiler identity` items under it. They used to be counted as lowering, so a warm cached build of mach showed `lower` at 1.5 s when no module was lowered. It now shows `cache` at 1.5 s and `lower` at 50 ms. A module restored while generating code, and the snapshot check that step repeats, are reported under `codegen` (#3519).
- A vector `::` compiles to packed instructions where they give exactly the result the scalar `::` gives for every lane, including NaN, the infinities and out-of-range values. Everywhere else it still converts one lane at a time, and a profile with `simd = "require"` refuses those conversions. x86_64 uses SSE2 only. Integer widening and narrowing are packed. So are f32↔f64, signed integers of up to 32 bits and unsigned integers of up to 16 bits to either float, and f32 or f64 to i8, i16 or i32. A float to any unsigned integer, u32 or u64 to a float, and i64 to or from a float stay per lane: the scalar path converts those through 64 bits, and SSE2 has no packed twin. SSE4.1's `pmovsx`/`pmovzx` wait on x86-64 levels (#3269). aarch64 packs every conversion with NEON (`sxtl`, `uxtl`, `xtn`, `fcvtl`, `fcvtn`, `scvtf`, `ucvtf`, `fcvtzs`, `fcvtzu`) except i64 and u64 to f32. A float lane that goes to an integer narrower than 64 bits saturates where the scalar `fcvtz*` saturates. SPIR-V packs integer widening and narrowing. Its float conversions stay per lane, because SPIR-V leaves their rounding and out-of-range results to the implementation. A 128-bit register holds two lanes of a conversion with a 64-bit side, so the new corpus case `vec/cast_lanes2` runs those pairs at two lanes, and the #3493 driver test compares them bit for bit against scalar `::` too (#3493).
### Fixed
- A build no longer refuses a dependency pinned to a release whose manifest selects one of its own dependencies by branch, commit or path, when the root declares that dependency itself. The root's selection replaces the release's, so the closure is still reproducible from the root. The rule from #3503 read the release's manifest text anyway. Version resolution in `mach dep add`, `update` and `outdated` skips such a selector the same way, while a release's `version` or `tag/` requirement on a root-declared identity still applies. A refusal now names both remedies: declare the dependency in the root manifest, or have the dependency cut a release that selects it by `version` or an exact `tag/`. `mach dep verify --release` checks the root's own manifest, which nothing exempts (#3553).
- A module-scope `$error` in the selected arm of a `$if` that reads another module's constant (`$if (cap.ON == 1) { ... } $or { $error("..."); }`) is reported, and it is the module's only diagnostic, as #3465 made it for gates resolve decides on its own. Such a gate is decided only in the gate fixpoint, and resolve's scan for a refusal ignored the decision the fixpoint recorded, so the `$error` was never reported and the names only the discarded arm binds were reported as unresolved. The scan now reads that decision, so the module is refused, the discarded arm's names and an importer's references to the module are not reported, and the selected arm still builds when the gate is true. The same holds when the constant is typed through a `def` alias (#3522).
- A test build (`mach test`) keeps the C link name and `#[library]` of an `ext` declared after a module-scope `$if` that only type layout decides, such as `$if ($size_of(Kevent) != 32) { $error(...); }`. A test build loads every module under `src`, and when such a module was first reached that way, the load walk stopped at the layout gate. The declarations after it were never registered, so a call to the `ext` was emitted under its module-qualified name, and a darwin link failed with `dynamic import 'std.system.os.darwin.libsystem.kqueue' has no #[library] attribution`. mach-std hit this once `std.allocator.page` gated its `use std.system.os`, which left `libsystem` to be loaded that way. The walk now goes on past a layout-decided gate, which declares nothing to load (#3523).
- vector `::` converts each lane numerically, like scalar `::`. Equal-size lanes were wrongly reinterpreted bit for bit; `:~` is the bit reinterpretation. Any pair of lane types converts, including `i32x4::f32x4` and `i8x4::i64x4`, which were refused because their sizes differ. A lane converts exactly as the scalar `::` of that lane does on the same target, including NaN, the infinities and out-of-range values. Both sides need the same lane count, so `i32x4::i64x2` is now an error; `i32x4:~i64x2` still reinterprets. The target vector catalog keys a conversion by its operand lane width too. `doc/language/operators.md` described the old reinterpretation and now states the lane-wise rule (#3493).
- `mach dep update <path> <name>` looks `<name>` up in the whole dependency closure, not only the root manifest. For a dependency the root doesn't declare, it checks the checkout out at the exact ref its requirer declares, so the remedy `mach dep verify` prints for a stale transitive checkout now runs and fixes it. Before, that command failed with `no dependency '<name>' in the manifest`, and the only way out was deleting the checkout by hand and pulling again. A name outside the closure is refused with `dependency '<name>' is not in the dependency closure`. A root-declared name and `--all` behave as before. The refusal from `verify` also names the chain that asks for the ref, as in `exact ref 'tag/v1.0.0' required by root -> a -> b` (#3494).
- Realizing a path dependency that lives in a git work tree copies what git keeps of it: its tracked files and the untracked files its ignore rules keep, listed by one `git ls-files`. An ignored file, directory or symlink is no longer copied, and an ignored symlink no longer makes realization fail with `path dependency inventory contains a symlink or non-regular entry`. A file that becomes ignored is removed from the copy on the next pull. A source outside a git work tree is copied as before (#3491).
...
- An operator applied to a generic type parameter resolves at the instantiation, against that instance's concrete type, rather than being refused during the pre-substitution check. Arithmetic, ordering, bitwise, shifts, unary `-` and `~`, `::` and `:~` casts, an unsuffixed literal beside a `T`-typed operand and a branch condition are all decided per instance, so `maxof[T]` and `sort[T]` are writable against `<` with no comparator parameter and `a == 0` works inside a generic. `:~` reads a type parameter through the instance and so agrees with `$size_of` instead of reading it as zero bytes. An instantiation at a type that does not support the operator is refused at the call that asked for it, naming that type, with the operator in the template attached as a related span. The template reports nothing of its own, so one bad instantiation is one refusal at one site, and a generic that nothing instantiates is not checked at all (#3432).
- `$is_integer` and `$is_float` complete the shape-predicate family beside `$is_pointer`, `$is_record`, `$is_secret`, `$is_tag` and `$is_union`. Both answer per instantiation and both answer false for a `^`-qualified operand, the way the rest of the family does (#3432).
### Changed
...
### Fixed
- A linked image's dead line-table sequences start at address 0, the way GNU ld and lld resolve them, instead of the `-1` dead-code tombstone the other debug sections keep. A sequence started at `-1` advances past the top of the address space into the addresses of real code, and GNU binutils before 2.43 (Ubuntu 24.04 ships 2.42) grows without bound decoding one: `addr2line` on a `-g` image with a discarded weak or duplicate function used all of a 16 GB machine, which is what killed the linux link legs in CI (#3466).
- A case construction whose positional payload is itself a case construction, such as `err[Cause].err{Cause.missing{}}`, stays bound when resolve runs again over the same module. A build with a top-level `$if` decided only in the gate fixpoint (a gate reading a constant typed through a `def` alias, whose branch declares something) resolves every module twice, and on the second pass the payload was left unbound without a diagnostic. That surfaced as `` `<error>` is not a tag `` in std's `std.types.error`, under `mach test` on 5.0.4 and under `mach build` on 5.1.0. Resolve now binds a literal the same way whichever form an earlier pass left it in (#3469).
- A module-scope `$error` in a selected arm is reported, and it is the only diagnostic its module produces. Resolve now decides the arms it can before anything else and reports the refusal there. Before, the directive was left to sema, which never ran once the refused module's `fwd` and `use` of the alias its missing arm would have bound failed to resolve, so std built for a freestanding target printed about 450 unresolved-name errors and never said the target was unsupported. A refused module binds nothing, sema does not analyze it, and an importer's reference to one of its names (a member, a `use` or a `fwd` of a symbol, and every use of the alias that bound) is not reported again. An arm whose gate cannot be decided until later is still reported by sema when it is decided (#3465).
- `mach check` announces each cell as checked rather than built, and an embed it cannot resolve says why. A cell's readout line takes its verb from the cell's goal, which the build goal catalog now carries beside the goal's display name, so a checked cell reads `checking app (host)` and only a built one reads `building`. When an `#[embed]` cannot read the path a `{artifact.<id>.out}` template resolved to, the diagnostic names the required artifact whose output that path is, and, when the invocation is one that writes no artifact, says so and says to build first. `mach check` still builds and generates nothing: a required artifact is checked, never built, so a consumer that embeds one is checked against a tree a build has already produced. `doc/language/manifest.md` states that contract where it documents artifact requirements (#3426).
...
- `mach fmt` takes a source file or a stream as well as a project. A directory or `mach.toml` operand walks that project's source as before, now reading only `[project].src`, so a project with no `[profile.*]` table formats. Any other file operand rewrites that one file with no manifest read. `-` reads source from stdin and writes its canonical layout to stdout, touching nothing on disk, with diagnostics on stderr and exit 1 for a malformed input. `--check` covers every operand form: nothing is written and each input whose layout differs is named on stdout, the stream as `<stdin>`. A lone `-` is a positional operand rather than an unknown flag for every command (#3419).
- `editor.invalidate` marks a loaded project stale, so the next analysis that needs it reloads the closure. `open`, `update` and `close` already do this for the buffers the editor session owns; this is the entry for a change it cannot see, a file written outside any buffer or a manifest edited on disk (#3431).
...
- A rejected query is one query's answer rather than the operation's, so a type error in one module no longer blanks the frontend products of the modules typed after it. A query whose compute reports through diagnostics is recorded as rejected under its own key: the queries that read it are rejected in turn, the ones that do not still compute, and every later reader of a rejected key is answered from that record instead of recomputing it, so its diagnostics are emitted once however many modules read it. The sema and resolve passes fold a rejected module and carry on over the rest. An internal failure still ends its operation where it met it, the operation's standing is still rejected, and a build still stops after the frontend (#3340).
...
- `editor.analyze` reuses a loaded project instead of reloading the whole import closure on every call. It marked the project dirty unconditionally, so `proj_ensure` re-ran `analyze_project_tolerant` over every module of every dependency per request; the flag now belongs to the operations that actually invalidate the project, and a load is also repeated when the request asks for a deeper phase than the loaded project holds or names different build options. A `PHASE_PARSE` analysis consults no project at all and answers from the buffer's own tree, so a syntax-only editor feature such as `textDocument/documentSymbol` pays no closure cost whether or not the file sits in a project (#3431).
- The line-table rows covering a function's prologue carry the function's own declaration rather than the location of the first instruction in its body, which at `-O2` belongs to an inlined callee in another file. A debugger, a symbolizer and an assembly view now resolve a function's entry address to its `fun` line, and `DW_AT_decl_file` and `DW_AT_decl_line`, which had the same origin, name the declaration too (#3394).
...
- A shared library exports the root project's `pub` declarations and the names its modules re-export with `fwd`, where it used to export every function and global it defined. A dependency's `pub` surface is no longer part of the library's ABI, `#[symbol]` chooses a name and not a visibility, and every other definition is hidden while still resolving inside the link. _Corrected 2026-09-17: this entry originally said "ELF `STV_HIDDEN`, Mach-O `N_PEXT`, absent from a PE export table". `STV_HIDDEN` holds only in the per-module objects. In the linked `.so`, those definitions are `LOCAL` symbols absent from `.dynsym`. No Mach-O or PE shared library is written at all yet (#3588). See #3581._ Dead private code can now be dropped from a shared object, and a shared artifact that exports nothing is refused by name (#3412).
- `mach test` no longer compiles the modules that only another target's artifacts reach. A module a selected-target artifact reaches, or that no artifact reaches, is collected as before, and one that belongs to another target's artifact is counted among the skipped modules (#3428).
...
- A profile with `debug = true` builds `spirv` targets, including shader artifacts a host artifact requires, where it refused them for having no debug model. The module carries the core SPIR-V debug instructions: `OpString`/`OpSource` for its files, `OpName`/`OpMemberName` for functions, variables, locals and block records, and `OpLine`/`OpNoLine` for its instructions (#3398).
...
- `driver.analyze_project_tolerant` answers the `Project` whenever the frontend ran, whatever its phases decided, with every module that reached a phase carrying that phase's product beside the diagnostics and the standing readable through `passes.frontend_status`; `err` is an operational failure only. `analyze_project` and `analyze_project_until` keep releasing the project on a rejected phase (#3337).
...
- An operating system declares the calling conventions it accepts per instruction set (`OsVTable.accepted_abis`, native first; `freestanding` declares itself unconstrained), replacing the single `native_abi`: `linux`/`darwin` with `win64` and `windows` with `sysv64` are refused at composition with `operating system '<os>' accepts calling convention '<abi>' on instruction set '<isa>', not '<abi>'` (tuple code `os_abi`, naming the whole set when it has several members), `linux` keeps `lp64d`, `lp64` and `lp64f` on `riscv64`, and `freestanding` keeps every convention the instruction set covers (#3320).
