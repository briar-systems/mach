...
- `mach dep add`, `mach dep remove`, and `mach dep update` validate before mutation and use Git’s native submodule and index operations. The candidate manifest is published last. A later failure reports completed work and preserves inspectable Git state. `update` alone moves selectors. A failed clone exits 3.
...
- A `path` dependency is copied into `dep/<id>` as ordinary files, never symlinked or automatically staged. Projects and local path dependencies build without a Git repository or index. Git dependencies in such projects use plain checkouts.
...
- Initialization reports rollback and cleanup failures independently and retains its recovery journal until every required operation, including source coordinator close, succeeds (#3174).
