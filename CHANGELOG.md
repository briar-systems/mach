...
### Changed
- `::` from an array to a vector, or back, requires the same element type and count. `[8]i16::i32x4` used to reinterpret the bytes of an equal-size array and is now an error that names the differing length. Spell a byte reinterpret `:~` (#3779).
