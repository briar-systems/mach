...
A patch release enabling position-independent executable (PIE) mode by default for `x86_64-darwin` targets (#2327).

### Fixed
- darwin: **PIE mode by default for `x86_64-darwin`.** Ensures `x86_64` macOS executables are linked as dynamic position-independent executables (`emit_dyn_exec`), matching modern macOS kernel and dyld requirements.
