...
Splitting it surfaced a second defect the single-artifact manifest had hidden (#2961). `mach build` enumerates artifact-by-target cells and filters them, so each platform gets its own stanza with nothing named on the command line, but `mach test` links the whole source tree and takes the first declared artifact as the root of that build. On Windows that is the non-Windows stanza, which declares no Windows target, so the run refuses. The two Windows `self test` steps in CI name `--bin mach-windows` until the selection is fixed, and the documentation says so where it recommends the split.
