...
# under qemu the loader comes from the leg's sysroot: MACH_QEMU_LD_PREFIX, or
# where Ubuntu's libc6-<arch>-cross installs it, which ci.yml installs for riscv64
produce_import_address() {
    ...
    case "$1" in
        ...
        qemu:*)
            case "$2" in
                riscv64-linux) prefix=${MACH_QEMU_LD_PREFIX:-/usr/riscv64-linux-gnu} ;;
                *) echo "link: import-address: no loader prefix known for leg '$2'" >&2; rm -f "$out"; return 2 ;;
            esac
            if [ ! -d "$prefix" ]; then
                echo "link: import-address: no $2 loader at '$prefix'; set MACH_QEMU_LD_PREFIX to a sysroot" >&2
                rm -f "$out"; return 2
            fi
