...
/* unit-case tags (mach #3400): unit cases have no payload, so the union holds only the
 * payload cases and each struct is the spelling a tag without the unit cases would have */
...
long long c_drives_mach_unit(void) {
