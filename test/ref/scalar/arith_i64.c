...
uint64_t checksum(uint64_t seed) {
    ...
    for (uint64_t j = 0; j < UINT64_C(8); j = (uint64_t)(j + UINT64_C(1))) {
        ...
        h = mix_i64(h, as_i64(b));
    }

    const uint64_t n = (uint64_t)(UINT64_C(0) - a);
    h = mix_i64(h, as_i64(n));
    h = mix_i64(h, as_i64((uint64_t)(UINT64_C(0) - n)));
    h = mix_i64(h, as_i64((uint64_t)(UINT64_C(0) - s)));
    h = mix_i64(h, as_i64((uint64_t)(UINT64_C(9223372036854775807) + a)));
    h = mix_i64(h, as_i64((uint64_t)(UINT64_C(9223372036854775808) - a)));
    h = mix_i64(h, as_i64((uint64_t)(UINT64_C(18446744073709551615) + a)));
