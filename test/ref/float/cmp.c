/* every comparison predicate over the special-value table at f32 and f64, then the
 * mixed-width form. one noinline part per width. */
#include "corpus.h"

static float bits_to_f32_cmp_f32(uint32_t b) {
    float f;
    memcpy(&f, &b, sizeof f);
    return f;
}

static uint64_t fold32_cmp_f32(uint64_t h, float v) {
    if (v != v) { return mix_u32(h, UINT32_C(0xFFFFFFFF)); }
    return mix_f32(h, v);
}

static uint64_t cmp_f32(uint64_t seed) {
    ...
    for (uint32_t k = 0; k < UINT32_C(12); k = (uint32_t)(k + UINT32_C(1))) {
        v[k] = bits_to_f32_cmp_f32((uint32_t)(pat[k] + s));
    }
    ...
    h = fold32_cmp_f32(h, lo);
    h = fold32_cmp_f32(h, hi);
...
static double bits_to_f64_cmp_f64(uint64_t b) {
    double f;
    memcpy(&f, &b, sizeof f);
    return f;
}

static float bits_to_f32_cmp_f64(uint32_t b) {
    float f;
    memcpy(&f, &b, sizeof f);
    return f;
}

static uint64_t fold64_cmp_f64(uint64_t h, double v) {
    if (v != v) { return mix_u64(h, UINT64_C(0xFFFFFFFFFFFFFFFF)); }
    return mix_f64(h, v);
}

static uint64_t cmp_f64(uint64_t seed) {
    uint64_t h = fold_init();
    const uint32_t s = (uint32_t)seed;
    ...
    for (uint64_t k = 0; k < UINT64_C(12); k = (uint64_t)(k + UINT64_C(1))) {
        v[k] = bits_to_f64_cmp_f64((uint64_t)(pat[k] + seed));
    }
    ...
    for (uint32_t n = 0; n < UINT32_C(8); n = (uint32_t)(n + UINT32_C(1))) {
        w[n] = bits_to_f32_cmp_f64((uint32_t)(nat[n] + s));
    }
    ...
    h = fold64_cmp_f64(h, lo);
    h = fold64_cmp_f64(h, hi);
...
uint64_t checksum(uint64_t seed) {
    uint64_t h = fold_init();
    h = mix_u64(h, cmp_f32(seed));
    h = mix_u64(h, cmp_f64(seed));
    return h;
}
