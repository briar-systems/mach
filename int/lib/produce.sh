...
emit_scan() {
    ...
    BEGIN { if (own != "") { ownpfx = "_M" length(own) own } }
    function demangle(s,   i, len, c, out) {
        if (substr(s, 1, 2) != "_M") { return s }
        i = 3
        out = ""
    ...
    /^[0-9a-f]+ <.*>:$/ {
        ...
        # that says whose function this is. a symbol carrying a literal name via
        # #[symbol(...)] has no project prefix and is excluded too, which is right,
        # since such a function is named for a foreign ABI rather than by this project.
