...
the 4.30 alias note was removed in 5.0.0 and a mismatch is refused
...
```mach
pub val STALE_LOCK_MSG: str = "mach.lock was removed in 5.0.0 and is refused
```

a `mach.lock` in the project root is refused: the committed gitlinks under
dep/ are the pins, and the 4.30 note that ignored the file was removed in 5.0.0

## fun refuse_stale_lock

```mach
pub fun refuse_stale_lock(alloc: *A.Allocator, root: str) err[outcome.Fail];
```
