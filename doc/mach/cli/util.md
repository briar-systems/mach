...
```mach
pub val DURATION_FORMS: str = "<n>ms, <n>s, <n>m or <n>h, with n a positive integer"
```

the accepted forms of a duration argument, for error messages

## fun parse_duration

```mach
pub fun parse_duration(s: *u8) opt[du.Duration];
```

parse a duration argument: a positive decimal integer followed by one of the
units ms, s, m or h
...
```mach
pub rec DurationText;
```

a duration in the largest unit of parse_duration's that divides it exactly
n:    the count
...
```mach
pub fun duration_text(d: du.Duration) DurationText;
```

render d in the form parse_duration reads back
